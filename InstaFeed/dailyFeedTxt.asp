...
<%
	...
	Dim savePath, rstMsg, strIdx, EndIdx, FileName


	savePath = server.mappath("/Files/InstaFeed/") + "\"
	...
	If strIdx = 1 Then
		FileName = "InstaFeed1.xml"
	End If

	If strIdx = 50000 Then
		FileName = "InstaFeed2.xml"
	End If

	If strIdx = 100000 Then
		FileName = "InstaFeed3.xml"
	End If

	If strIdx = 150000 Then
		FileName = "InstaFeed4.xml"
	End If

	If strIdx = 200000 Then
		FileName = "InstaFeed5.xml"
	End If

	If strIdx = 250000 Then
		FileName = "InstaFeed6.xml"
	End If

	If strIdx = 300000 Then
		FileName = "InstaFeed7.xml"
	End If

	If strIdx = 350000 Then
		FileName = "InstaFeed8.xml"
	End If

	If strIdx = 400000 Then
		FileName = "InstaFeed9.xml"
	End If

	If strIdx = 450000 Then
		FileName = "InstaFeed10.xml"
	End If	

	If strIdx = 500000 Then
		FileName = "InstaFeed11.xml"
	End If		

	If strIdx = 550000 Then
		FileName = "InstaFeed12.xml"
	End If

	If strIdx = 600000 Then
		FileName = "InstaFeed13.xml"
	End If

	If strIdx = 650000 Then
		FileName = "InstaFeed14.xml"
	End If

	If strIdx = 700000 Then
		FileName = "InstaFeed15.xml"
	End If

	If strIdx = 750000 Then
		FileName = "InstaFeed16.xml"
	End If

	If strIdx = 800000 Then
		FileName = "InstaFeed17.xml"
	End If

	If strIdx = 850000 Then
		FileName = "InstaFeed18.xml"
	End If

	If strIdx = 900000 Then
		FileName = "InstaFeed19.xml"
	End If

	If strIdx = 950000 Then
		FileName = "InstaFeed20.xml"
	End If

	If strIdx >= 1000000 Then
		FileName = "InstaFeed21.xml"
	End If				

	If strIdx = 283235 Then
		FileName = "InstaFeedEtc.xml"
	End If
	...
	If Not(rsCTget.bof Or rsCTget.eof) Then
		...
		Channel.childnodes(2).childnodes(0).text = "10x10 Instagram Dynamic Ads Feed"  '설명
		...
		Do Until rsCTget.eof
			...
			'xItem.childnodes(4).childnodes(0).text = "https://tenten.app.link/3p?%243p=a_facebook&branch_ad_format=Product&%24ios_url="&Server.URLEncode("http://m.10x10.co.kr/category/category_itemprd.asp?itemid="&rsCTget("itemid")&"&utm_source=instagram&utm_medium=referral&utm_campaign=posting&utm_term=producttag&rdsite=producttag")&"&%24android_url="&Server.URLEncode("http://m.10x10.co.kr/category/category_itemprd.asp?itemid="&rsCTget("itemid")&"&utm_source=instagram&utm_medium=referral&utm_campaign=posting&utm_term=producttag&rdsite=producttag")&"&%24desktop_url="&Server.URLEncode("http://www.10x10.co.kr/shopping/category_prd.asp?itemid="&rsCTget("itemid")&"&utm_source=instagram&utm_medium=referral&utm_campaign=posting&utm_term=producttag&rdsite=producttag")&"&%24deeplink_path="&Server.URLEncode("m.10x10.co.kr/apps/appCom/wish/web2014/category/category_itemprd.asp?itemid="&rsCTget("itemid")&"&utm_source=instagram&utm_medium=referral&utm_campaign=posting&utm_term=producttag&rdsite=producttag&isbranch=true")&"&~feature=paid+advertising&%24uri_redirect_mode=1&%24ios_redirect_timeout=1000&%24ios_passive_deepview=false&%24android_passive_deepview=false&~ad_id={{ad.id}}&~ad_name={{ad.name}}&~ad_set_id={{adset.id}}&~ad_set_name={{adset.name}}&~campaign=posting&~campaign_id={{campaign.id}}&~keyword="&rsCTget("itemid")
			...
			'// url encode 제거
			'xItem.childnodes(4).childnodes(0).text = "https://tenten.app.link/3p?$3p=a_facebook&branch_ad_format=Product&$ios_url=http://m.10x10.co.kr/category/category_itemprd.asp?itemid="&rsCTget("itemid")&"&utm_source=instagram&utm_medium=referral&utm_campaign=posting&utm_term=producttag&rdsite=producttag&$android_url=http://m.10x10.co.kr/category/category_itemprd.asp?itemid="&rsCTget("itemid")&"&utm_source=instagram&utm_medium=referral&utm_campaign=posting&utm_term=producttag&rdsite=producttag&$desktop_url=http://www.10x10.co.kr/shopping/category_prd.asp?itemid="&rsCTget("itemid")&"&utm_source=instagram&utm_medium=referral&utm_campaign=posting&utm_term=producttag&rdsite=producttag&$deeplink_path=m.10x10.co.kr/apps/appCom/wish/web2014/category/category_itemprd.asp?itemid="&rsCTget("itemid")&"&utm_source=instagram&utm_medium=referral&utm_campaign=posting&utm_term=producttag&rdsite=producttag&isbranch=true&~feature=paid+advertising&$uri_redirect_mode=1&$ios_redirect_timeout=1000&$ios_passive_deepview=false&$android_passive_deepview=false&~ad_id={{ad.id}}&~ad_name={{ad.name}}&~ad_set_id={{adset.id}}&~ad_set_name={{adset.name}}&~campaign=posting&~campaign_id={{campaign.id}}&~keyword="&rsCTget("itemid")

			'//인스타 링크 일반 링크로 변경
			xItem.childnodes(4).childnodes(0).text = "https://m.10x10.co.kr/category/category_itemprd.asp?itemid="&rsCTget("itemid")&"&utm_source=instagram&utm_medium=producttag&utm_campaign=product_traffic&utm_term="&rsCTget("itemid")&"&rdsite=inst"
...
'		rstMsg = "데이터 파일 [InstaFeed.xml] 생성 완료"
	Set xmlPars = Nothing
