...
<%
response.write "TTT"  '' 2016/10/10 ''2016/12/12 ''2020-07-11(진영)
response.end
...
if (stDt="") then
	if (iparseType=4) then
		stDt = LEFT(dateadd("h",-1,now()),10) & " "
		if (Hour(Now()) = 0) then
			stDt = stDt & "23:00:00"
		else
			stDt = stDt & Right("0" & (Hour(Now()) - 1),2)& ":00:00"
		end if
    elseif (iparseType=3) then  ''complexType
        if (hour(now())="7") then
            stDt = dateadd("h",-3,now())  ''7=>4
        else
...
if (edDt="") then
    if (iparseType=4) then
		edDt = LEFT(dateadd("h",-1,now()),10) & " "
		if (Hour(Now()) = 0) then
			edDt = edDt & "23:59:59"
		else
			edDt = edDt & Right("0" & (Hour(Now()) - 1),2)& ":59:59"
		end if
    elseif (iparseType=3) then
        if (hour(now())="7") then
...
response.write "<br>"
response.write "qTime="&qTime&"|parseTime="&parseTime&"|pagename=/anal/es_GetData.asp"
%>
<% else %>
