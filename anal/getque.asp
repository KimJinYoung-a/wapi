...
dim excelyn
	excelyn = requestcheckvar(request("excelyn"),1)

''<!-- #include virtual="/lib/util/aspJSON1.17.asp" -->

if excelyn="Y" then		'/엑셀출력
...
C_ALLOWIPLIST = Array(  "115.94.163.42","115.94.163.43","115.94.163.44","115.94.163.45","115.94.163.46" _
						,"115.91.233.126" _
...
C_ALLOWREFERERLIST = Array ("http://testscm.10x10.co.kr","https://testscm.10x10.co.kr","http://scm.10x10.co.kr","https://scm.10x10.co.kr","https://webadmin.10x10.co.kr","http://webadmin.10x10.co.kr","http://testwebadmin.10x10.co.kr" )
...
function getDbDataJson(kind,startdate,enddate,dimensions,channel,param2,param3,pretype,ordtype,shdate,shmakerid,byref iretJson, excelyn)
    ...
    select CASE kind
        ...
        CASE "meachulall"  ''매출
            if (param2="master") then
                if (pretype<>"") then
                    istrSql = "[db_analyze_data_raw].[dbo].[sp_TEN_getOrderMData_WithPre]('"&startdate&"','"&enddate&"','"&shdate&"','"&dimensions&"','"&channel&"','"&pretype&"','"&ordtype&"',"&MAXResult&")"
                else
                    istrSql = "[db_analyze_data_raw].[dbo].[sp_TEN_getOrderMData]('"&startdate&"','"&enddate&"','"&shdate&"','"&dimensions&"','"&channel&"','"&ordtype&"',"&MAXResult&")"
                end if
            elseif (param2="dispcate") then
                igroupHeaderResult = getGroupHeaderResult("XXX", isGroupHeader) '' 그룹이 아님..
                istrSql = "[db_analyze_data_raw].[dbo].[sp_TEN_get_MD_DispCateSell]('"&startdate&"','"&enddate&"','"&dimensions&"','"&channel&"','"&shmakerid&"','"&ordtype&"',"&MAXResult&")"
            elseif (param2="dispcatemaechul") then
                igroupHeaderResult = getGroupHeaderResult("XXX", isGroupHeader) '' 그룹이 아님..
                istrSql = "[db_analyze_data_raw].[dbo].[sp_TEN_get_MD_DispCatemaechul_all]('"&startdate&"','"&enddate&"','"&shdate&"','"&dimensions&"','"&channel&"','"&shmakerid&"','"&ordtype&"',"&MAXResult&")"
                'istrSql = "[db_analyze_data_raw].[dbo].[sp_TEN_get_MD_DispCatemaechul]('"&startdate&"','"&enddate&"','"&shdate&"','"&dimensions&"','"&channel&"','"&shmakerid&"','"&ordtype&"',"&MAXResult&")"
                'istrSql = "[db_analyze_data_raw].[dbo].[sp_TEN_get_MD_DispCatemaechul_withpre]('"&startdate&"','"&enddate&"','"&shdate&"','"&dimensions&"','"&channel&"','"&shmakerid&"','"&ordtype&"',"&MAXResult&")"
            elseif (param2="dispcategroupmaechul") then
                igroupHeaderResult = getGroupHeaderResult("XXX", isGroupHeader) '' 그룹이 아님..
                istrSql = "[db_analyze_data_raw].[dbo].[sp_TEN_get_MD_DispCategroupmaechul_all]('"&startdate&"','"&enddate&"','"&shdate&"','"&dimensions&"','"&channel&"','"&shmakerid&"','"&ordtype&"',"&MAXResult&")"
        ...
        CASE "mktall" '' 마케팅관련
            ...
            elseif (param2="cruser") then
                istrSql = "[db_analyze_data_raw].[dbo].[sp_TEN_get_MKT_CR_BY_USERS]('"&startdate&"','"&enddate&"','"&dimensions&"','"&channel&"','"&ordtype&"',"&MAXResult&")"
            elseif (param2="crsession") then
                istrSql = "[db_analyze_data_raw].[dbo].[sp_TEN_get_MKT_CR_BY_SESSION]('"&startdate&"','"&enddate&"','"&dimensions&"','"&channel&"','"&ordtype&"',"&MAXResult&")"
            elseif (param2="crelkip") then
                istrSql = "[db_analyze_data_raw].[dbo].[sp_TEN_get_MKT_CR_BY_ELK_UniqIP]('"&startdate&"','"&enddate&"','"&dimensions&"','"&channel&"','"&ordtype&"',"&MAXResult&")"
            ...
            elseif (param2="mailzine") then
                igroupHeaderResult = getGroupHeaderResult("XXX", isGroupHeader) '' 그룹이 아님..
                istrSql = "[db_analyze_data_raw].[dbo].[sp_TEN_get_mailzine]('"&startdate&"','"&enddate&"','"&shdate&"','"&dimensions&"','"&ordtype&"',"&MAXResult&")"
            else
                CALL retErrorJoson("'"&param2&"' param2 not defined")
            end if
    ...
    Dim buf, ifieldtype, bufFieldName, bufdataAttr, bufFieldGrpName, tmpFieldGrpName, tmpFieldGrpCount, tmpFieldCount
    	tmpFieldGrpCount=0
    	tmpFieldCount=0
	...
	'/엑셀출력
	if excelyn="Y" then
		iVal="<html><head><title></title></head><meta http-equiv='Content-Type' content='text/html;charset=euc-kr' /><body>" & vbcrlf
		iVal=iVal&"<table width='100%' align='center' cellpadding='3' cellspacing='1' bgcolor='#999999' border=1>" & vbcrlf
	    if (iAnaleRs.Fields.count>0) then
			'/////////////////// 헤더 뿌린다 ///////////////////////////////////
			'/그룹헤더
			if isGroupHeader then
				iVal=iVal&"<tr align='center' bgcolor='#E6E6E6'>" & vbcrlf
				...
				if (NOT (bufdataAttr="HIDDEN")) then
					'/셀 병합의 상단라인 처리
					if LEFT(iField.Name,4)="GRP_" then
						if tmpFieldGrpName=bufFieldGrpName then
							tmpFieldGrpCount=tmpFieldGrpCount+1
						else
							if tmpFieldGrpName<>"" then
								iVal=iVal&"		<td colspan="& tmpFieldGrpCount+1 &">" & vbcrlf
								iVal=iVal&"			"& tmpFieldGrpName &"" & vbcrlf
								iVal=iVal&"		</td>" & vbcrlf
							end if

							tmpFieldGrpName=bufFieldGrpName
							tmpFieldGrpCount=0
						end if

						if iAnaleRs.Fields.count=iFieldCount then
							if tmpFieldGrpName<>"" then
								iVal=iVal&"		<td colspan="& tmpFieldGrpCount+1 &">" & vbcrlf
								iVal=iVal&"			"& tmpFieldGrpName &"" & vbcrlf
								iVal=iVal&"		</td>" & vbcrlf
					...
					'/셀 병합의 하단라인 처리
						...
						iVal=iVal&"		<td rowspan=2>" & vbcrlf
						iVal=iVal&"			"& bufFieldName &"" & vbcrlf
						iVal=iVal&"		</td>" & vbcrlf
					end if
				end if
	    ...
	    if (not iAnaleRs.EOF) then
	        Do Until iAnaleRs.Eof
				iVal=iVal&"<tr align='center' bgcolor='#FFFFFF'>" & vbcrlf
	            for i=0 to iFieldCount-1
	                if NOT (LEFT(iAnaleRs.Fields(i).name,6)="hidden") then
	                	tmpFieldCount = tmpFieldCount + 1
	                    ifieldtype=iAnaleRs.Fields(i).Type

	                    if ifieldtype="131" then
	                    	iVal=iVal&"		<td align='right'>"
	                    else
	                    	iVal=iVal&"		<td>"
	                    end if

	                    if ifieldtype="131" then
	                    	iVal=iVal&"			"& CurrFormat(iAnaleRs(i)) &""
	                    else
	                ...
	                if tmpFieldCount>=iFieldCount then
	                	tmpFieldCount=0
						iVal=iVal&"</tr>" & vbcrlf
					end if
	            next
...
dim shmakerid : shmakerid = request("shmakerid")
...
Dim iretJson
if (getDbDataJson(kind,startdate,enddate,dimensions,channel,param2,param3,pretype,ordtype,shdate,shmakerid,iretJson, excelyn)) then
    '' response.write iretJson
