...
Dim C_isGroupHeader : C_isGroupHeader= Array("userjoin","meachul","meachulbylevel","mktall") ''
...
function getDbDataJson(kind,startdate,enddate,dimensions,channel,param2,param3,pretype,ordtype,byref iretJson)
    ...
    select CASE kind
        ...
        CASE "mktall" '' 마케팅관련
            if (param2="gavisit") then
                istrSql = "[db_analyze_data_raw].[dbo].[sp_TEN_get_MKT_GAvisit]('"&startdate&"','"&enddate&"','"&dimensions&"','"&channel&"','"&ordtype&"',"&MAXResult&")"
            ...
            elseif (param2="meachul") then
                istrSql = "[db_analyze_data_raw].[dbo].[sp_TEN_get_MKT_Meachul]('"&startdate&"','"&enddate&"','"&dimensions&"','"&channel&"','"&ordtype&"',"&MAXResult&")"
        ...
        Do Until iAnaleRs.Eof
            ...
            for i=0 to iFieldCount-1
                if NOT (LEFT(iAnaleRs.Fields(i).name,6)="hidden") then
                    ...
                    if (ifieldtype=3 or ifieldtype=6 or ifieldtype=131) then
...
if (getDbDataJson(kind,startdate,enddate,dimensions,channel,param2,param3,pretype,ordtype,iretJson)) then
    '' response.write iretJson
    response.write "callback("&iretJson&")"
end if
