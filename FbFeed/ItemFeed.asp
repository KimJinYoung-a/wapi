...
<%
	...
	Dim savePath, rstMsg, strIdx, EndIdx


	savePath = server.mappath("/FbFeed/") + "\"
...
'    end if
	...
	If strIdx=1 Then
		...
		query1 = query1 + " 		(Select top 1 catename From db_AppWish.dbo.tbl_display_cate Where catecode = substring(cast(ci.catecode as nvarchar(max)), 1, 6)) "
		query1 = query1 + " 	else "
		query1 = query1 + " 		null "
		query1 = query1 + " 	end as cate2, "
		query1 = query1 + " 	case when len(ci.catecode)>9 then "
		query1 = query1 + " 		(Select top 1 catename From db_AppWish.dbo.tbl_display_cate Where catecode = substring(cast(ci.catecode as nvarchar(max)), 1, 9)) "
		query1 = query1 + " 	else "
		query1 = query1 + " 		null "
		query1 = query1 + " 	end as cate3, "
		query1 = query1 + " 	case when len(ci.catecode)>12 then "
		query1 = query1 + " 		(Select top 1 catename From db_AppWish.dbo.tbl_display_cate Where catecode = substring(cast(ci.catecode as nvarchar(max)), 1, 12)) "
		query1 = query1 + " 	else "
		query1 = query1 + " 		null "
		query1 = query1 + " 	end as cate4, "
		query1 = query1 + " 	case when len(ci.catecode)>15 then "
		query1 = query1 + " 		(Select top 1 catename From db_AppWish.dbo.tbl_display_cate Where catecode = substring(cast(ci.catecode as nvarchar(max)), 1, 15)) "
		query1 = query1 + " 	else "
		query1 = query1 + " 		null "
		...
		query1 = query1 + " 	inner join db_AppWish.[dbo].[tbl_display_cate_item] ci on i.itemid = ci.itemid And ci.isdefault = 'y' "
		query1 = query1 + " 	Where i.isusing='Y' And i.itemid <> 0 And i.sellyn = 'Y' And ci.depth>=3  And (c.sellcount>0 or datediff(day, i.regdate, getdate())<=20) And i.itemid <> '1513445' "
		query1 = query1 + " )Tot "
		If strIdx<>"" Then
		...
		If Not(rsCTget.bof Or rsCTget.eof) Then
			...
			Do Until rsCTget.eof
				...
				xItem.childnodes(1).childnodes(0).text = rsCTget("itemname")  '상품명
				...
				xItem.childnodes(2).childnodes(0).text = "생활감성채널 텐바이텐 - " & Replace(rsCTget("itemname") & " " & Trim(rsCTget("designercomment")),"""","")  '상품설명
				...
				xItem.childnodes(4).childnodes(0).text = "http://www.10x10.co.kr/shopping/category_prd.asp?itemid="&rsCTget("itemid")  '링크값(일단은 pc로 보내면 자동으로 모바일로 갈테니 pc주소로 보냄)
				...
				xAppLink.setAttribute "content","tenwishapp://http://m.10x10.co.kr/apps/appCom/wish/web2014/category/category_itemprd.asp?itemid="&rsCTget("itemid")
		...
		'// XML파일 저장
		xmlPars.save(savePath & "FaceBookFeed.xml")
'		rstMsg = "데이터 파일 [FaceBookFeed.xml] 생성 완료"
	...
	Else
		...
		query1 = query1 + " 	inner join db_AppWish.dbo.tbl_user_c m on i.makerid = m.userid "
		query1 = query1 + " 	inner join db_AppWish.[dbo].[tbl_display_cate_item] ci on i.itemid = ci.itemid And ci.isdefault = 'y' "
		query1 = query1 + " 	Where i.isusing='Y' And i.itemid <> 0 And i.sellyn = 'Y' And ci.depth>=3 And (c.sellcount>0 or datediff(day, i.regdate, getdate())<=20) "
		query1 = query1 + " )Tot "
		If strIdx<>"" Then
			query1 = query1 + " Where num >= "&strIdx&" And num < "&EndIdx&" "
		End If
...
'			response.ContentType="text/xml"
			...
			Set xmlPars = Server.CreateObject("Msxml2.DOMDocument")
			xmlPars.Async = "False"
			xmlPars.Load(savePath & "FaceBookFeed2.xml")


			Dim objRoot
			Set objRoot = xmlPars.documentElement '// 현 xml 루트데이터

			Do Until rsCTget.eof

				Set xItem = xmlPars.CreateElement("item")
				objRoot.getElementsByTagName("channel")(0).appendchild(xItem) 
		...
		xmlPars.save(savePath & "FaceBookFeed2.xml")
'		rstMsg = "데이터 파일 [FaceBookFeed.xml] 생성 완료"
		Set xmlPars = Nothing
		Set xAppLink = Nothing
		Set xItem = Nothing
