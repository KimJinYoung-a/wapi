...
<%
    ...
    if isArray(arrRst) then
		...
		for i=0 to ubound(arrRst,2)
			...
			xItem.childnodes(1).childnodes(0).text = Trim(Replace(arrRst(2,i), Chr(8), ""))  '상품명
			...
			xItem.childnodes(2).childnodes(0).text = "생활감성채널 텐바이텐 - " & stripHTML(Trim(Replace(Replace(Replace(Replace(arrRst(2,i), """", ""), Chr(32), ""), "", ""), Chr(8), "")))  '상품설명
			...
			xItem.childnodes(4).childnodes(0).text = "http://www.10x10.co.kr/shopping/category_prd.asp?itemid="&arrRst(1,i)  '링크값(일단은 pc로 보내면 자동으로 모바일로 갈테니 pc주소로 보냄)
			...
			'xItem.childnodes(8).childnodes(0).text = FormatNumber(arrRst(6,i),0)&" KRW"  '가격(sellcash값 보내줌)
			xItem.childnodes(8).childnodes(0).text = FormatNumber(arrRst(7,i),0)&" KRW" '가격(orgprice값 보내줌) 2021년 4월 7일 마케팅 요청으로 변경
			...
			xItem.childnodes(10).childnodes(0).text = Replace(arrRst(5,i), Chr(8), "")  '브랜드명
			...
			If ((arrRst(8,i)="Y") and (arrRst(7,i)-arrRst(6,i)>0)) or (arrRst(26,i) = "Y") And isCouponPriceDisplay(arrRst(1,i)) Then
				...
				xItem.childnodes(12).childnodes(0).text = FormatNumber(GetCouponAssignPrice(arrRst(6,i), GetCouponDiscountPrice(arrRst(28,i), arrRst(29,i), arrRst(6,i)), arrRst(26,i)),0)&" KRW" '가격(할인값 보내줌)

				'// 할인기간 셋팅
				''xItem.AppendChild(xmlPars.CreateElement("g:sale_price_effective_date")) 
				''xItem.childnodes(13).appendChild(xmlPars.createCDATASection("name_Cdata"))
				''xItem.childnodes(13).childnodes(0).text = "2021-03-29T00:00+09:00/2021-04-25T23:59+09:00"
			End If
			...
			xAppLink.setAttribute "content","tenwishapp://http://m.10x10.co.kr/apps/appCom/wish/web2014/category/category_itemprd.asp?itemid="&arrRst(1,i)
