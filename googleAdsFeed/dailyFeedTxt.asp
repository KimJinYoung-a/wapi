...
<%
'' 구글 ADS 파일 Make / 일별
Const MaxPage   = 200		'140 -> 20(10만개)
Const PageSize = 5000

Dim appPath : appPath = server.mappath("/googleAdsFeed/") + "\"
Dim FileName: FileName = "googleAdsFeed_temp.txt"
Dim newFileName: newFileName = "googleAdsFeed.txt"
Dim fso, tFile

Function WriteMakeGooleFeedFile(tfso, arrList, byref iLastItemid)
    Dim intLoop, iRow, strSql
    Dim bufstr, isMake
    Dim itemid, deliv, lp, barcode, ArrCateNM, vLink, vMobLink, vImageLink, vAddImageArr, vSplitAddImage, vSplitGubun, additional_image_link
	...
	For intLoop=0 to iRow
		...
		itemname		= Replace(itemname,"""","")
		vLink			= "https://tenten.app.link/3p?%243p=a_google_adwords&branch_ad_format=Product&%24ios_url="&Server.URLEncode("http://m.10x10.co.kr/category/category_itemprd.asp?itemid="&itemid&"&utm_source=google&utm_medium=ad&utm_campaign=retarget&utm_term=gdn&rdsite=gdn")&"&%24android_url="&Server.URLEncode("http://m.10x10.co.kr/category/category_itemprd.asp?itemid="&itemid&"&utm_source=google&utm_medium=ad&utm_campaign=retarget&utm_term=gdn&rdsite=gdn")&"&%24desktop_url="&Server.URLEncode("http://www.10x10.co.kr/shopping/category_prd.asp?itemid="&itemid&"&utm_source=google&utm_medium=ad&utm_campaign=retarget&utm_term=gdn&rdsite=gdn")&"&%24deeplink_path="&Server.URLEncode("m.10x10.co.kr/apps/appCom/wish/web2014/category/category_itemprd.asp?itemid="&itemid&"&utm_source=google&utm_medium=ad&utm_campaign=retarget&utm_term=gdn&rdsite=gdn&isbranch=true")&"&~feature=paid+advertising&%24uri_redirect_mode=1&%24ios_redirect_timeout=1000&%24ios_passive_deepview=false&%24android_passive_deepview=false&~ad_id={{ad.id}}&~ad_name={{ad.name}}&~ad_set_id={{adset.id}}&~ad_set_name={{adset.name}}&~campaign=dpa&~campaign_id={{campaign.id}}"
		vMobLink		= "https://tenten.app.link/3p?%243p=a_google_adwords&branch_ad_format=Product&%24ios_url="&Server.URLEncode("http://m.10x10.co.kr/category/category_itemprd.asp?itemid="&itemid&"&utm_source=google&utm_medium=ad&utm_campaign=retarget&utm_term=gdn&rdsite=gdn")&"&%24android_url="&Server.URLEncode("http://m.10x10.co.kr/category/category_itemprd.asp?itemid="&itemid&"&utm_source=google&utm_medium=ad&utm_campaign=retarget&utm_term=gdn&rdsite=gdn")&"&%24desktop_url="&Server.URLEncode("http://www.10x10.co.kr/shopping/category_prd.asp?itemid="&itemid&"&utm_source=google&utm_medium=ad&utm_campaign=retarget&utm_term=gdn&rdsite=gdn")&"&%24deeplink_path="&Server.URLEncode("m.10x10.co.kr/apps/appCom/wish/web2014/category/category_itemprd.asp?itemid="&itemid&"&utm_source=google&utm_medium=ad&utm_campaign=retarget&utm_term=gdn&rdsite=gdn&isbranch=true")&"&~feature=paid+advertising&%24uri_redirect_mode=1&%24ios_redirect_timeout=1000&%24ios_passive_deepview=false&%24android_passive_deepview=false&~ad_id={{ad.id}}&~ad_name={{ad.name}}&~ad_set_id={{adset.id}}&~ad_set_name={{adset.name}}&~campaign=dpa&~campaign_id={{campaign.id}}"
        ...
        bufstr = bufstr & "in stock" & vbTab '// 모바일 링크는 사용할 필요가 없으므로 제거
...
sqlStr = sqlStr & " VALUES ('googleAdsFeed_ST')"
...
IF FTotCnt > 0 THEN
    ...
    Set fso = CreateObject("ADODB.Stream")
		...
		For i=0 to FTotPage-1
			...
			sqlStr = ""
			sqlStr = sqlStr & " INSERT INTO [db_outmall].[dbo].tbl_nate_scraplog (ref) "
			sqlStr = sqlStr & " VALUES ('googleAdsFeed_"&(i+1)*PageSize&"_"&iLastItemid&"')"
			dbCTget.execute sqlStr
		NExt
...
sqlStr = ""
sqlStr = sqlStr & " INSERT INTO [db_outmall].[dbo].tbl_nate_scraplog (ref) "
sqlStr = sqlStr & " VALUES ('googleAdsFeed_ED')"
dbCTget.execute sqlStr
