...
'        end if
    ...
    public function getLimitLotteEa()
        dim ret
        ret = Foptlimitno - Foptlimitsold - 5
		...
		If Not(rsget.EOF or rsget.BOF) Then
			...
			Do until rsget.EOF
				If (rsget("disptpcd")="12") then                        ''전문몰 카테고리를 기본으로 하라고.. /2012/06/14
				    strRst = strRst & "&disp_no_b=" & rsget("dispNo")		'기본 전시카테고리
				Else
				    IF (ogrpCode=rsget("groupCode")) then
    				    strRst = strRst & "&disp_no=" & rsget("dispNo") 	'추가 전시카테고리
    				End IF
			    End If
				rsget.MoveNext
				i = i + 1
...
'response.end
    ...
    Public Function getLotteOptionParamToEdit()
		...
		If (FLimitYn = "Y") Then
			...
			ret = ret & "&inv_qty="&getOptionLimitNo()			'재고관리 할 경우 단품이 있어야..
		Else
		...
		''rw ret
		If (getLotteSellYn = "Y") and (getOptionLimitNo > 0) Then										 																'판매상태			(*:10:판매,20:품절)
	...
	public Function getLotteComItemRegParameter(isEdit)
		...
		strRst = strRst & "&goods_nm=" & Server.URLEncode(Fsocname_kor) & " " & Server.URLEncode(Trim(getItemOptNameFormat))
		...
		strRst = strRst & "&sale_prc=" & cLng(GetRaiseValue(FRealSellprice/10)*10)														'판매가(원판매가)	(*) 소비자가로 설정한 이유..?
		strRst = strRst & "&mrgn_rt=12" 																								'마진율				(*:11%) ==> 2013/01/01 12%
		strRst = strRst & "&pur_prc=" & cLng(FRealSellprice*0.88)																		'공급가				(*)
		...
		strRst = strRst & "&corp_goods_no=" & FNewItemid																				'업체상품번호(텐텐상품코드)
	...
	Public Function getLotteComItemEditParameter()
		...
		strRst = strRst & "&brnd_no=" & tenBrandCd																						'브랜드코드			(*)
		strRst = strRst & "&goods_nm=" & Server.URLEncode(Fsocname_kor) & " " & Server.URLEncode(Trim(getItemOptNameFormat))			'상품명				(*)
		strRst = strRst & getItemKeyword																								'키워드 //2013-08-09 김진영 수정
...
Class CLotte
	...
	Public Sub getLotteNotRegOneItem
		...
		strSql = strSql & " JOIN db_etcmall.[dbo].[tbl_Outmall_option_Manager] as M on M.itemid = o.itemid and M.itemoption = o.itemoption and M.mallid = 'lotteCom' and M.idx = '"&FRectIdx&"' "
		strSql = strSql & " LEFT JOIN db_etcmall.dbo.tbl_lotteAddOption_regItem as R on R.midx = M.idx "
		...
		strSql = strSql & "	and M.idx not in (Select midx From db_etcmall.dbo.tbl_lotteAddOption_regItem where lottestatCD not in ('00','10') ) "				'롯데등록상품 제외  ,'10' -- 오류
	...
	Public Sub getLotteEditOneItem
		...
		strSql = strSql & " SELECT TOP " & FPageSize & " i.*, o.itemoption, o.isusing as optisusing, o.optsellyn, o.optlimitno, o.optlimitsold, o.optaddprice, o.optionname "
		strSql = strSql & "	, c.keywords, c.ordercomment, c.sourcearea, c.makername, c.usingHTML, c.itemcontent, isNULL(c.requireMakeDay,0) as requireMakeDay, uc.socname_kor "
		strSql = strSql & "	, R.LotteGoodNo, R.LotteTmpGoodNo, R.LotteSellYn "
		strSql = strSql & "	, R.accFailCNT, R.lastErrStr, R.Lotteprice, R.regImagename "
		...
		strSql = strSql & " JOIN db_item.dbo.tbl_item_option as o on i.itemid = o.itemid "
		strSql = strSql & " JOIN db_etcmall.[dbo].[tbl_Outmall_option_Manager] as M on M.itemid = o.itemid and M.itemoption = o.itemoption and M.mallid = 'lotteCom' and M.idx = '"&FRectIdx&"' "
		strSql = strSql & " JOIN db_etcmall.dbo.tbl_lotteAddoption_regitem as R on R.midx = M.idx "
		...
		strSql = strSql & " and isNULL(R.LotteTmpGoodNo,R.LotteGoodNo) is Not Null "
		rsget.CursorLocation = adUseClient
		rsget.Open strSql, dbget, adOpenForwardOnly, adLockReadOnly
		FResultCount = rsget.RecordCount
		FTotalCount = FResultCount
...
Function getLotteGoodno(iidx)
	Dim strSql
	strSql = ""
	strSql = strSql & " SELECT TOP 1 lotteGoodno FROM db_etcmall.[dbo].[tbl_lotteAddOption_regItem] WHERE midx = '"&iidx&"' "
	rsget.CursorLocation = adUseClient
	rsget.Open strSql, dbget, adOpenForwardOnly, adLockReadOnly
