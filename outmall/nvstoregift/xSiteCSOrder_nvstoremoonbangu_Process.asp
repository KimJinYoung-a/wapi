...
If (mode = "getxsitecslist") Then
    If (sellsite="nvstoregift") Then
		...
		For i = 0 To 1
			Call getNvstoregiftChangeOrder(oServ, occd, i, oinputDT)
		Next
		'############################################################
    End If
End If
...
Function getLastCSInputDT()
	...
	sqlStr = sqlStr&" where sellsite = 'nvstoregift'  "
...
Public Sub getNvstoregiftChangeOrder(iServ, iccd, lp, iinputDT)
	...
	Dim nvstoregiftURL
	If (application("Svr_Info") = "Dev") Then
		nvstoregiftURL = "http://sandbox.api.naver.com/ShopN/"&iServ
	Else
		nvstoregiftURL = "http://ec.api.naver.com/ShopN/"&iServ
	End If
...
'							rw "------------------------------------------------------------" & lp
							...
							strSql = " IF NOT EXISTS (SELECT idx FROM db_temp.dbo.tbl_xSite_TMPCS WHERE SellSite = 'nvstoregift' and OutMallOrderSerial = '" & CStr(OutMallOrderSerial) & "' and OrgDetailKey = '" & CStr(OrgDetailKey) & "' and CSDetailKey = '" & CStr(CSDetailKey) & "') "
							strSql = strSql & " BEGIN "
							...
							strSql = strSql & " 	('"&getDivCD&"', '단순변심', 'nvstoregift', '" & html2db(CStr(OutMallOrderSerial)) & "', '', '', '', '', '', "
							strSql = strSql & "		'', '', '', '', '', '' "
					...
					strSql = strSql + " 	and c.sellsite = 'nvstoregift' "
					''rw strSql
					dbget.Execute strSql
				End If
			End If
