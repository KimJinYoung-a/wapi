...
If (mode = "getxsitecslist") Then
    If (sellsite="nvstoremoonbangu") Then
		...
		For i = 0 To 1
			Call getNvstoremoonbanguChangeOrder(oServ, occd, i, oinputDT)
		Next
		'############################################################
    End If
End If
...
Function getLastCSInputDT()
	...
	sqlStr = sqlStr&" where sellsite = 'nvstoremoonbangu'  "
...
Public Sub getNvstoremoonbanguChangeOrder(iServ, iccd, lp, iinputDT)
	...
	If (application("Svr_Info") = "Dev") Then
		nvstoremoonbanguURL = "http://sandbox.api.naver.com/ShopN/"&iServ
	Else
		nvstoremoonbanguURL = "http://ec.api.naver.com/ShopN/"&iServ
	End If
...
'							rw "------------------------------------------------------------" & lp
							...
							strSql = " IF NOT EXISTS (SELECT idx FROM db_temp.dbo.tbl_xSite_TMPCS WHERE SellSite = 'nvstoremoonbangu' and OutMallOrderSerial = '" & CStr(OutMallOrderSerial) & "' and OrgDetailKey = '" & CStr(OrgDetailKey) & "' and CSDetailKey = '" & CStr(CSDetailKey) & "') "
							strSql = strSql & " BEGIN "
							...
							strSql = strSql & " 	('"&getDivCD&"', '단순변심', 'nvstoremoonbangu', '" & html2db(CStr(OutMallOrderSerial)) & "', '', '', '', '', '', "
							strSql = strSql & "		'', '', '', '', '', '' "
					...
					strSql = strSql + " 	and c.sellsite = 'nvstoremoonbangu' "
					''rw strSql
					dbget.Execute strSql
				End If
			End If
