...
'		sqlStr = sqlStr & " and getdate() >= startDate and getdate() <= endDate "
	...
	'// 스토어팜 판매여부 반환
	Public Function getNvstoremoonbanguSellYn()
		'판매상태 (10:판매진행, 20:품절)
		If FSellYn="Y" and FIsUsing="Y" then
			If FLimitYn = "N" or (FLimitYn = "Y" and FLimitNo - FLimitSold >= CMAXLIMITSELL) then
				getNvstoremoonbanguSellYn = "Y"
			Else
				getNvstoremoonbanguSellYn = "N"
			End If
		Else
			getNvstoremoonbanguSellYn = "N"
		End If
	...
	Public Function getLimitNvstoremoonbanguEa()
		Dim ret
		If FLimitYn = "Y" Then
		...
		If (ret < 1) Then ret = 0
		getLimitNvstoremoonbanguEa = ret
	End Function
...
'			buf = buf & "					<shop:Importer></shop:Importer>"							'수입사명 | 수입산인 경우 필수
	...
	Public Function getNvstoremoonbanguCertInfo
...
'			End If
			getNvstoremoonbanguCertInfo = buf
		Else
			getNvstoremoonbanguCertInfo = ""
		End If
	End Function
	...
	Public Function getImageType()
		...
		basicimgStr	= ""
		strSql = ""
		strSql = strSql & " SELECT TOP 10 imgType, storefarmURL FROM db_etcmall.[dbo].[tbl_nvstoremoonbangu_Image] WHERE itemid = '"&FItemid&"' "
		rsget.CursorLocation = adUseClient
		rsget.Open strSql, dbget, adOpenForwardOnly, adLockReadOnly
		If not rsget.Eof Then
...
'		buf = buf & "					<shop:IndividualCustomUniqueCode></shop:IndividualCustomUniqueCode>"	'개인통관 고유부호 수집 여부 Y or N | 미입력시 Y로 저장
	...
	Public Function getNvstoremoonbanguItemContParamToReg()
		...
		strRst = strRst & ("<p><center><a href=""http://storefarm.naver.com/tenbytenclass"" target=""_blank""><img src=""http://fiximage.10x10.co.kr/web2008/etc/top_notice_nvstorefarm.jpg""></a></center></p><br>")
		...
		strRst = strRst & ("</div>")
		getNvstoremoonbanguItemContParamToReg = strRst
...
'		buf = buf & "				<shop:SellerCustomCode2></shop:SellerCustomCode2>"				'판매자가 내부에서 사용하는 코드
	...
	Public Function getNvstoremoonbanguItemInfoCdToReg
		...
		strSQL = strSQL & " 	SELECT TOP 1 itemid, certNum "
		strSQL = strSQL & " 	FROM db_item.[dbo].[tbl_safetycert_tenReg] "
		strSQL = strSQL & " 	WHERE itemid = '"&FItemid&"' "
		strSQL = strSQL & " ) TR on i.itemid = TR.itemid "
...
'			    End If
		...
		End If
		rsget.Close
		getNvstoremoonbanguItemInfoCdToReg = buf
	End Function

	'// 업로드 이미지 XML 생성
	Public Function getNvstoremoonbanguImageRegXML(oServ, oOper)
		...
		getNvstoremoonbanguImageRegXML = strRst
	End Function

	'// 상품등록 XML 생성
	Public Function getNvstoremoonbanguItemRegXML(oServ, oOper, isEdit)
		...
		If isEdit = "Y" Then
			strRst = strRst & "			<shop:ProductId>"&FNvstoremoonbanguGoodNo&"</shop:ProductId>"		'상품ID | 없으면 등록, 있으면 수정
		End If
...
'		strRst = strRst & "				<shop:AttributeValueList></shop:AttributeValueList>"		' ,로 분리된 속성의 목록 | 현재는 사용하지 않으며 향후 사용 예정
		If FNeedCert = "Y" Then
			strRst = strRst & getNvstoremoonbanguCertInfo()
		End If
		If FNeedCert = "Y" Then
			If getNvstoremoonbanguCertInfo = "" Then
...
'		strRst = strRst & "				<shop:ValidDate></shop:ValidDate>"							'유효 일자 | YYYY-MM-DD 형식
		...
		strRst = strRst & "				<shop:DetailContent><![CDATA["&getNvstoremoonbanguItemContParamToReg&"]]></shop:DetailContent>"		'#상품 상세 정보
...
'		strRst = strRst & "				<shop:SalePrice>"&Clng(GetRaiseValue(MustPrice/10)*10)&"</shop:SalePrice>"		'#판매가
		...
		If (isEdit = "Y")  Then
			If (Foptioncnt = 0) Then
				strRst = strRst & "				<shop:StockQuantity>"&getLimitNvstoremoonbanguEa&"</shop:StockQuantity>"		'#재고 수량 | 상품등록시 필수, 상품 수정시 재고 수량을 입력하지 않으면 스토어팜 DB에 저장된 현재 재고값이 변하지 않는다. 수정시 재고 수량 0으로 입력되면 StatusType으로 전달된 항목은 무시되며 상품 상태는 OSTK(품절)로 저장됨
			End If
		Else
			strRst = strRst & "				<shop:StockQuantity>"&getLimitNvstoremoonbanguEa&"</shop:StockQuantity>"		'#재고 수량 | 상품등록시 필수, 상품 수정시 재고 수량을 입력하지 않으면 스토어팜 DB에 저장된 현재 재고값이 변하지 않는다. 수정시 재고 수량 0으로 입력되면 StatusType으로 전달된 항목은 무시되며 상품 상태는 OSTK(품절)로 저장됨
		End If
...
'		strRst = strRst & "				<shop:WifiOnly></shop:WifiOnly>"		'Wifi 전용 상품 여부 | Y or N..태블릿 카테고리 상품인 경우 필수..Y 입력시 PurchaseApplicationUrl, CellPhonePrice 입력불가..N 입력시 PurchaseApplicationUrl, CellPhonePrice 입력 필수
		...
		strRst = strRst & getNvstoremoonbanguItemInfoCdToReg
		...
		strRst = strRst & "	</soapenv:Body>"
		strRst = strRst & "</soapenv:Envelope>"
		getNvstoremoonbanguItemRegXML = strRst
	End Function
End Class
...
Class CNvstoremoonbangu
	...
	Public Sub getNvstoremoonbanguNotRegOneItem
		Dim strSql, addSql, i
		...
		strSql = strSql & "	, isNULL(R.nvstoremoonbanguStatCD,-9) as nvstoremoonbanguStatCD"
		...
		strSql = strSql & "	,isNull(v.itemid, 0) as nvstorefarmid "
		strSql = strSql & " FROM db_item.dbo.tbl_item as i "
		...
		strSql = strSql & " LEFT JOIN db_etcmall.[dbo].[tbl_nvstoremoonbangu_regItem] R on i.itemid = R.itemid"
		strSql = strSql & " LEFT JOIN db_etcmall.[dbo].[tbl_nvstorefarm_regItem] v on i.itemid = v.itemid "
		...
		strSql = strSql & " and convert(varchar(6), i.cate_large + i.cate_mid) = '035021' "	'여행/취미(035) > 애완용품(021)만 등록 // 2021-02-24 김진영
		...
		strSql = strSql & " and not exists(SELECT top 1 n.makerid FROM [db_temp].dbo.tbl_jaehyumall_not_in_makerid n with (nolock) WHERE n.makerid=i.makerid and n.mallgubun = 'nvstoremoonbangu') "
		strSql = strSql & " and not exists(SELECT top 1 n.itemid FROM [db_temp].dbo.tbl_jaehyumall_not_in_itemid n with (nolock) WHERE n.itemid=i.itemid and n.mallgubun = 'nvstoremoonbangu') "
		strSql = strSql & " and not exists(select top 1 y.idx FROM db_temp.[dbo].[tbl_jaehyumall_not_in_category] y with (nolock) "
		strSql = strSql & " 				WHERE convert(varchar(6), y.cdl + y.cdm) = convert(varchar(6), (i.cate_large + i.cate_mid)) and y.mallgubun = 'nvstoremoonbangu') "
		If FRectGubun <> "IMG" Then
			strSql = strSql & "	and i.itemid not in (Select itemid From db_etcmall.[dbo].[tbl_nvstoremoonbangu_regItem] where nvstoremoonbanguStatCD > 3) "
		End If
		...
		If not rsget.EOF Then
			Set FOneItem = new CNvstoremoonbanguItem
                ...
                FOneItem.FNvstoremoonbanguStatCD	= rsget("nvstoremoonbanguStatCD")
				...
				FOneItem.FNvstorefarmid 	= rsget("nvstorefarmid")

		End If
		rsget.Close
	End Sub

	Public Sub getNvstoremoonbanguEditOneItem
		Dim strSql, addSql, i
		If FRectItemID <> "" Then
		...
		strSql = strSql & "	, isNULL(m.nvstoremoonbanguGoodNo, '') as nvstoremoonbanguGoodNo, m.nvstoremoonbanguprice, m.nvstoremoonbanguSellYn, isNULL(m.regedOptCnt, 0) as regedOptCnt "
...
'		strSql = strSql & "		or ((i.sailyn = 'N') and ( ((i.sellcash-i.buycash)/i.sellcash)*100 < "&CMAXMARGIN&" )) "
		...
		strSql = strSql & " 	or exists(SELECT top 1 n.makerid FROM [db_temp].dbo.tbl_jaehyumall_not_in_makerid n with (nolock) WHERE n.makerid=i.makerid and n.mallgubun = 'nvstoremoonbangu') "
		strSql = strSql & " 	or exists(SELECT top 1 n.itemid FROM [db_temp].dbo.tbl_jaehyumall_not_in_itemid n with (nolock) WHERE n.itemid=i.itemid and n.mallgubun = 'nvstoremoonbangu') "
		...
		strSql = strSql & " FROM db_item.dbo.tbl_item as i "
		strSql = strSql & " JOIN db_item.dbo.tbl_item_contents as c on i.itemid = c.itemid "
		strSql = strSql & " JOIN db_etcmall.dbo.tbl_nvstoremoonbangu_regItem as m on i.itemid = m.itemid "
		strSql = strSql & " LEFT JOIN db_user.dbo.tbl_user_c uc on i.makerid = uc.userid"
		strSql = strSql & " LEFT JOIN db_etcmall.dbo.tbl_nvstorefarm_cate_mapping as bm on bm.tenCateLarge=i.cate_large and bm.tenCateMid=i.cate_mid and bm.tenCateSmall=i.cate_small "
		strSql = strSql & " LEFT JOIN db_etcmall.dbo.tbl_nvstorefarm_category as ca on ca.catekey = bm.catekey "
		strSql = strSql & " WHERE 1 = 1"
		strSql = strSql & " and m.APIaddImg = 'Y' "
		strSql = strSql & " and m.nvstoremoonbanguStatCD = 7 "
		strSql = strSql & addSql
		strSql = strSql & " and m.nvstoremoonbanguGoodNo is Not Null "									'#등록 상품만
		rsget.CursorLocation = adUseClient
		rsget.Open strSql, dbget, adOpenForwardOnly, adLockReadOnly
		FResultCount = rsget.RecordCount
		FTotalCount = FResultCount
		If not rsget.EOF Then
			Set FOneItem = new CNvstoremoonbanguItem
				...
				FOneItem.FUsingHTML			= rsget("usingHTML")
				FOneItem.Fitemcontent		= db2html(rsget("itemcontent"))
				FOneItem.FnvstoremoonbanguGoodNo		= rsget("nvstoremoonbanguGoodNo")
				FOneItem.FNvstoremoonbanguprice		= rsget("nvstoremoonbanguprice")
				FOneItem.FNvstoremoonbanguSellYn		= rsget("nvstoremoonbanguSellYn")
...
Function getNvstoremoonbanguGoodNo(iitemid)
	Dim strSql
	strSql = ""
	strSql = strSql & " SELECT TOP 1 nvstoremoonbanguGoodNo FROM db_etcmall.[dbo].[tbl_nvstoremoonbangu_regItem] WHERE itemid = '"&iitemid&"' "
	rsget.CursorLocation = adUseClient
	rsget.Open strSql, dbget, adOpenForwardOnly, adLockReadOnly
		getNvstoremoonbanguGoodNo = rsget("nvstoremoonbanguGoodNo")
	rsget.Close
End Function
