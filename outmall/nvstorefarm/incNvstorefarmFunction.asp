...
'이미지 업로드
Public Function fnNvstorefarmImageReg(iitemid, strParam, byRef iErrStr, ichgImageNm, iservice, ioperation)
 		...
 		If objXML.Status = 200 Then
			Set xmlDOM = Server.CreateObject("MSXML.DOMDocument")
				...
				If ResponseType = "SUCCESS" Then
					...
					strSql = strSql & " db_etcmall.[dbo].[tbl_nvstorefarm_Image] " & vbcrlf
					...
					For Each SubNodes in imglist
						...
						If InStr(myURL, "/basic/") OR InStr(myURL, "/nvadd1/") Then
							strSql = ""
							strSql = strSql & " INSERT INTO db_etcmall.[dbo].[tbl_nvstorefarm_Image] (itemid, imgType, tenURL, storefarmURL) VALUES " & vbcrlf
					...
					Set imglist = nothing
					strSql = ""
					strSql = strSql & " UPDATE db_etcmall.[dbo].[tbl_nvstorefarm_regItem] SET "
...
'상품 둥록
Public Function fnNvstorefarmItemReg(iitemid, strParam, byRef iErrStr, iSellCash, iNvstorefarmSellYn, ilimityn, ilimitno, ilimiysold, iitemname, iimageNm, iservice, ioperation, ichkXML)
	Dim objXML, xmlDOM, strSql, iMessage, nvstorefarmURL, SubNodes, ResponseType, imglist
	Dim ProductId
	If (application("Svr_Info") = "Dev") Then
		nvstorefarmURL = "http://sandbox.api.naver.com/ShopN/"&iservice
...
'				If ResponseType = "SUCCESS" Then
				...
				If ResponseType = "SUCCESS" AND ProductId <> "" Then
					...
					strSql = strSql & " SET nvstorefarmGoodNo = '"&ProductId&"' " & vbcrlf
					strSql = strSql & " , nvstorefarmLastUpdate = getdate() " & vbcrlf
					strSql = strSql & " , nvstorefarmPrice = " & iSellCash & vbcrlf
					strSql = strSql & " , accFailCnt = 0 " & vbcrlf
					strSql = strSql & " , nvstorefarmRegdate = getdate() " & vbcrlf
					strSql = strSql & " , nvstorefarmStatCd = 7 " & vbcrlf
					strSql = strSql & "	FROM db_etcmall.[dbo].[tbl_nvstorefarm_regItem] R " & vbcrlf
...
'상품 수정
Public Function fnNvstorefarmItemEDIT(iitemid, strParam, byRef iErrStr, iSellCash, iNvstorefarmSellYn, ilimityn, ilimitno, ilimiysold, iitemname, ichgImageNm, iservice, ioperation)
	Dim objXML, xmlDOM, strSql, iMessage, nvstorefarmURL, SubNodes, ResponseType, imglist
	Dim ProductId
	If (application("Svr_Info") = "Dev") Then
		nvstorefarmURL = "http://sandbox.api.naver.com/ShopN/"&iservice
 		...
 		If objXML.Status = 200 Then
			Set xmlDOM = Server.CreateObject("MSXML.DOMDocument")
				...
				If (session("ssBctID")="kjy8517") Then
					rw "수정전문 : <textarea cols=40 rows=10>"&strParam&"</textarea>" 
				End If
				...
				If ResponseType = "SUCCESS" Then
					...
					strSql = ""
					strSql = strSql & " UPDATE R " & vbcrlf
					strSql = strSql & " SET nvstorefarmLastUpdate = getdate() " & vbcrlf
					strSql = strSql & " , nvstorefarmPrice = " & iSellCash & vbcrlf
					strSql = strSql & " , accFailCnt = 0 " & vbcrlf
...
End Function
'옵션 등록
Public Function fnNvstorefarmOptionReg(iitemid, strParam, byRef iErrStr, iservice, ioperation)
	Dim objXML, xmlDOM, strSql, iMessage, nvstorefarmURL, SubNodes, ResponseType, imglist
...
'상품 조회
Public Function fnNvstorefarmItemSearch(iitemid, strParam, byRef iErrStr, iservice, ioperation)
	Dim objXML, xmlDOM, strSql, iMessage, nvstorefarmURL, SubNodes, ResponseType, imglist
	Dim myURL, yourURL, statusType, nvRegitemname, MasterPrice, ProductId
	If (application("Svr_Info") = "Dev") Then
		nvstorefarmURL = "http://sandbox.api.naver.com/ShopN/"&iservice
	...
	Set objXML = Server.CreateObject("MSXML2.ServerXMLHTTP.3.0")
		...
		If (session("ssBctID")="kjy8517") Then
			rw "조회 : <textarea cols=40 rows=10>"&objXML.responseText&"</textarea>"
		End If
 		...
 		If objXML.Status = 200 Then
			Set xmlDOM = Server.CreateObject("MSXML.DOMDocument")
				...
				If ResponseType = "SUCCESS" Then
					...
					strSQL = ""
					strSQL = strSQL & " UPDATE R" & VbCRLF
					strSQL = strSQL & " SET nvstorefarmPrice = " & MasterPrice & VbCRLF
					strSQL = strSQL & " ,nvstorefarmSellyn='"&statusType&"'" & VbCRLF
					strSQL = strSQL & " ,regitemname='"&html2db(nvRegitemname)&"'" & VbCRLF
					strSQL = strSQL & " ,lastStatCheckDate = getdate()" & VbCRLF
					strSQL = strSQL & " ,nvstorefarmGoodNo = isNULL(R.nvstorefarmGoodNo,'"&ProductId&"')"&VbCRLF
					strSQL = strSQL & " FROM db_etcmall.[dbo].[tbl_nvstorefarm_regItem] R" & VbCRLF
					strSQL = strSQL & " where R.itemid="&iitemid & VbCRLF
					strSQL = strSQL & " and isNULL(nvstorefarmGoodNo,'') in ('','"&ProductId&"')"&VbCRLF    ''중복등록된CaSE 대비
					strSQL = strSQL & " and (isNULL(nvstorefarmPrice,0)<>"&MasterPrice&"" & VbCRLF
					strSQL = strSQL & "     or isNULL(nvstorefarmSellyn,'')<>'"&statusType&"'"& VbCRLF
					strSQL = strSQL & "     or isNULL(regitemname,'')<>'"&html2db(nvRegitemname)&"'"& VbCRLF
					strSQL = strSQL & "     or isNULL(nvstorefarmGoodNo,'')<>'"&ProductId&"'"& VbCRLF
					strSQL = strSQL & " )"
				    dbget.Execute strSQL
...
'옵션 조회
Public Function fnNvstorefarmOptionSearch(iitemid, strParam, byRef iErrStr, iservice, ioperation)
	Dim objXML, xmlDOM, strSQL, iMessage, nvstorefarmURL, SubNodes, ResponseType, imglist
	Dim myURL, yourURL, statusType, nvRegitemname, MasterPrice, ProductId
...
'판매 상태 변경
Public Function fnNvstorefarmSellyn(iitemid, ichgSellYn, strParam, byRef iErrStr, iservice, ioperation)
	Dim objXML, xmlDOM, strSql, iMessage, nvstorefarmURL, SubNodes, ResponseType, imglist
	Dim myURL, yourURL
	If (application("Svr_Info") = "Dev") Then
		nvstorefarmURL = "http://sandbox.api.naver.com/ShopN/"&iservice
...
'response.end
				...
				If ResponseType = "SUCCESS" Then
					strSql = ""
					strSql = strSql & " UPDATE db_etcmall.[dbo].[tbl_nvstorefarm_regItem] " & VbCRLF
					strSql = strSql & " SET nvstorefarmSellYn = '"&ichgSellYn&"'" & VbCRLF
					strSql = strSql & " ,nvstorefarmLastUpdate = getdate()" & VbCRLF
					strSql = strSql & " ,accFailCNT=0" & VbCRLF
					strSql = strSql & " WHERE itemid = "&iitemid
					dbget.Execute(strSql)
				...
				Else
					...
					If InStr(iMessage, "품절 상품은 판매중 상태로만 변경할 수 있습니다") OR InStr(iMessage, "판매금지 상태의") Then
						strSql = ""
						strSql = strSql & " UPDATE db_etcmall.[dbo].[tbl_nvstorefarm_regItem] " & VbCRLF
						strSql = strSql & " SET nvstorefarmSellYn = 'N'" & VbCRLF
						strSql = strSql & " ,nvstorefarmLastUpdate = getdate()" & VbCRLF
						strSql = strSql & " ,accFailCNT=0" & VbCRLF
						strSql = strSql & " WHERE itemid = "&iitemid
						dbget.Execute(strSql)
...
'상품 삭제
Public Function fnNvstorefarmDelete(iitemid, strParam, byRef iErrStr, iservice, ioperation)
	Dim objXML, xmlDOM, strSql, iMessage, nvstorefarmURL, SubNodes, ResponseType, imglist
	Dim myURL, yourURL
	If (application("Svr_Info") = "Dev") Then
		nvstorefarmURL = "http://sandbox.api.naver.com/ShopN/"&iservice
 		...
 		If objXML.Status = 200 Then
			Set xmlDOM = Server.CreateObject("MSXML.DOMDocument")
				...
				If ResponseType = "SUCCESS" Then
					...
					strSql = strSql &" SELECT TOP 1 'nvstorefarm', i.itemid, r.nvstorefarmGoodNo, r.nvstorefarmregdate, getdate(), r.lastErrStr" & VBCRLF
					strSql = strSql &" FROM db_item.dbo.tbl_item as i " & VBCRLF
					strSql = strSql &" JOIN db_etcmall.dbo.tbl_nvstorefarm_regItem as r on i.itemid = r.itemid " & VBCRLF
					strSql = strSql &" WHERE i.itemid = "&iitemid & VBCRLF
					dbget.Execute(strSql)

					strSql = ""
					strSql = strSql & " DELETE FROM [db_etcmall].[dbo].[tbl_nvstorefarm_regItem] " & vbcrlf
					strSql = strSql & " WHERE itemid = '"&iitemid&"' "
					dbget.Execute(strSql)

					strSql = ""
					strSql = strSql & " DELETE FROM [db_etcmall].[dbo].[tbl_nvstorefarm_Image] " & vbcrlf
					strSql = strSql & " WHERE itemid = '"&iitemid&"' "
					dbget.Execute(strSql)
...
End Function
'상품 상태 변경 XML
Public Function getNvstorefarmSellynParameter(invstorefarmgoodno, ichgSellYn, iservice, ioperation)
	...
	strRst = strRst &"				<shop:ProductId>"&invstorefarmgoodno&"</shop:ProductId>"
	strRst = strRst &"				<shop:StatusType>"&stopYN&"</shop:StatusType>"
	strRst = strRst &"			</SaleStatus>"
	strRst = strRst &"		</shop:ChangeProductSaleStatusRequest>"
	strRst = strRst &"	</soapenv:Body>"
	strRst = strRst &"</soapenv:Envelope>"
	getNvstorefarmSellynParameter = strRst
End Function

'상품 옵션 등록 XML
Public Function getNvstorefarmOptionRegXML(iitemid, invstorefarmgoodno, iservice, ioperation)
...
'					End If
		...
		strRst = strRst &"	</soapenv:Body>"
		strRst = strRst &"</soapenv:Envelope>"
		getNvstorefarmOptionRegXML = strRst
	Else
		Dim isRegedOptCnt
		strSql = ""
		strSql = strSql &"  SELECT TOP 1 isnull(regedOptcnt, 0) as regedOptcnt "
		strSql = strSql & " FROM db_etcmall.[dbo].[tbl_nvstorefarm_regItem]"
		strSql = strSql & " WHERE nvstorefarmStatcd = 7 and itemid=" & iitemid
		rsget.CursorLocation = adUseClient
		rsget.Open strSql, dbget, adOpenForwardOnly, adLockReadOnly
		If Not(rsget.EOF or rsget.BOF) then
		...
		If (ioptioncnt = 0) and (isRegedOptCnt > 0) Then
			...
			getNvstorefarmOptionRegXML = strRst
		Else
			getNvstorefarmOptionRegXML = "X"
		End If
	End If
End Function

'상품 조회 XML
Public Function getNvstorefarmItemSearchParameter(invstorefarmgoodno, iservice, ioperation)
	...
	strRst = strRst &"			<shop:Version>2.0</shop:Version>"
	strRst = strRst &"			<SellerId>"&reqID&"</SellerId>"
	strRst = strRst &"			<ProductId>"&invstorefarmgoodno&"</ProductId>"
	strRst = strRst &"		</shop:GetProductRequest>"
	strRst = strRst &"	</soapenv:Body>"
	strRst = strRst &"</soapenv:Envelope>"
	getNvstorefarmItemSearchParameter = strRst
End Function

'옵션 조회 XML
Public Function getNvstorefarmOptionSearchParameter(invstorefarmgoodno, iservice, ioperation)
	Dim strRst, oaccessLicense, oTimestamp, osignature, reqID
	If (application("Svr_Info") = "Dev") Then
		reqID = "qa2tc329"
	Else
	...
	strRst = strRst &"	</soapenv:Body>"
	strRst = strRst &"</soapenv:Envelope>"
	getNvstorefarmOptionSearchParameter = strRst
End Function
...
'상품 삭제 XML
Public Function getNvstorefarmDeleteParameter(invstorefarmgoodno, iservice, ioperation)
	Dim strRst, oaccessLicense, oTimestamp, osignature, reqID
	If (application("Svr_Info") = "Dev") Then
		reqID = "qa2tc329"
	Else
	...
	strRst = strRst &"	</soapenv:Body>"
	strRst = strRst &"</soapenv:Envelope>"
	getNvstorefarmDeleteParameter = strRst
End Function
...
Public Function getAddressBookList(iccd)
	...
	Dim nvstorefarmURL
	If (application("Svr_Info") = "Dev") Then
		nvstorefarmURL = "http://sandbox.api.naver.com/ShopN/"&oServ
	Else
		nvstorefarmURL = "http://ec.api.naver.com/ShopN/"&oServ
	End If
				...
				If xmlDOM.getElementsByTagName("n:Category").length > 0 Then
					Set Nodes = xmlDOM.getElementsByTagName("n:Category")
						For each SubNodes in Nodes
							...
							strSql = ""
							strSql =  strSql & " IF Exists(SELECT * FROM db_etcmall.dbo.tbl_nvstorefarm_category WHERE catekey = '"&categoryId&"') "
							strSql =  strSql & " 	BEGIN "
							strSql =  strSql & " 		UPDATE db_etcmall.dbo.tbl_nvstorefarm_category SET "
							...
							strSql =  strSql & " ELSE "
							strSql =  strSql & " 	BEGIN "
							strSql =  strSql & " 		INSERT INTO db_etcmall.dbo.tbl_nvstorefarm_category "
				...
				If xmlDOM.getElementsByTagName("n:ExceptionalCategoryList").length > 0 Then
					Set Nodes = xmlDOM.getElementsByTagName("n:ExceptionalCategory")
						For each SubNodes in Nodes
							...
							strSql = ""
							strSql =  strSql & " INSERT INTO db_etcmall.[dbo].[tbl_nvstorefarm_certInfo] (CateKey, certCode, certName, lastUpdate) "
