...
Class CNvstorefarmItem
	...
	Public Function fnIsSpecialDate
		Dim sqlStr, specialPrice, cnt, cnt2, cnt3
		sqlStr = ""
...
'		sqlStr = sqlStr & " and getdate() >= startDate and getdate() <= endDate "
		...
		sqlStr = sqlStr & " WHERE mallgubun = '"& CMALLNAME &"' "
		sqlStr = sqlStr & " and itemid = '"& Fitemid &"' "
		sqlStr = sqlStr & " and getdate() >= orgpricestartDate and getdate() <= orgpriceendDate "
		rsget.CursorLocation = adUseClient
		rsget.Open sqlStr, dbget, adOpenForwardOnly, adLockReadOnly
		If Not(rsget.EOF or rsget.BOF) Then
			cnt3 = rsget("CNT")
	...
	'// 스토어팜 판매여부 반환
	Public Function getNvstorefarmSellYn()
		'판매상태 (10:판매진행, 20:품절)
		If FSellYn="Y" and FIsUsing="Y" then
			If FLimitYn = "N" or (FLimitYn = "Y" and FLimitNo - FLimitSold >= CMAXLIMITSELL) then
				getNvstorefarmSellYn = "Y"
			Else
				getNvstorefarmSellYn = "N"
			End If
		Else
			getNvstorefarmSellYn = "N"
		End If
	...
	Public Function getLimitNvstorefarmEa()
		Dim ret
		If FLimitYn = "Y" Then
	...
	' 	Dim nowDateTime, isSugiItemid
	' 	nowDateTime = FormatDate(now(), "0000-00-00 00:00:00")
	' 	Select Case FItemid
	' 		Case "4917571", "4908794", "4908793", "4877096", "4877095", "4907998", "4922388", "5013330", "4975399", "4735685", "4922735", "4803913", "4368389", "4953097", "4952084", "4923890", "4820641", "5045206", "5045204", "5014645", "5014644", "4976799", "4976809", "4976815", "4976823", "4894017", "4958612", "5051949", "4856248", "4888943", "4547180", "4547181", "4976531", "4287540", "2555017", "4915798", "4993171", "4907340", "4242172", "2595556", "4958683", "4980713", "4196764", "4490128", "5027118", "4235776", "4228008", "4977125", "2529345", "2529343", "4676888", "4933815", "4933816", "4934010", "2225241", "1712092", "1712107", "4114000", "3362673", "4172386", "4890241", "4963848", "4963852", "4814002", "4814022", "4313607", "4568684", "4051526", "3977150", "3977151", "4524679", "4546794", "4628339", "3794864", "4628340", "4730635", "4409965", "4459573", "4479968", "3747451", "4023002", "4023005", "3747452", "4023016", "4023017", "4023024", "4568685", "4568683", "3664921", "4003592", "3917795", "3917796", "3917797", "3957850", "3957863", "3957883", "4414849", "3504305", "3523266", "3581268", "4951517", "4906064", "4571317", "4571386", "4571674", "4571728", "4571888", "4504295", "4504298", "4504301", "4504308", "4504334", "4414788", "4414793", "4499368", "4499369", "4499370", "4499371", "4503622", "4503627", "4824966", "2784157", "2785591", "2953019", "3019191", "3019218", "3313868", "3471382", "3471386", "3471393", "3649588", "4460500", "4883156", "4495193", "4495197", "4572790", "4572867", "4572966", "4442989", "4442990", "3806476", "4814040", "3806482", "4814043", "4460277", "4824678"
	' 			isSugiItemid = "Y"
	' 		Case Else 
	' 			isSugiItemid = "N"
	' 	End Select

	' 	If (nowDateTime > "2022-12-14 14:40:00" AND nowDateTime < "2022-12-15 23:59:00") AND isSugiItemid = "Y" Then
	' 		getSalePrice = Forgprice
	' 	Else
	' 		If mustCnt > 0 Then
	' 			getSalePrice = Forgprice
	' 		Else
	' 			'2022-12-14 김진영 하단 주석처리
	...
	Public Function getSalePrice
		Dim sqlStr, mustCnt, orgpricestartDate, orgpriceendDate, startDate, endDate
		mustCnt = 0
		sqlStr = ""
		sqlStr = sqlStr & " SELECT orgpricestartDate, orgpriceendDate, startDate, endDate "
		sqlStr = sqlStr & " FROM db_etcmall.[dbo].[tbl_outmall_mustPriceItem] "
		sqlStr = sqlStr & " WHERE mallgubun = '"& CMALLNAME &"' "
		sqlStr = sqlStr & " and itemid = '"& Fitemid &"' "
'		sqlStr = sqlStr & " and getdate() >= startDate and getdate() <= endDate "
		...
		If Not(rsget.EOF or rsget.BOF) Then
			mustCnt = 1
			orgpricestartDate = rsget("orgpricestartDate")
			orgpriceendDate = rsget("orgpriceendDate")
			startDate = rsget("startDate")
			endDate = rsget("endDate")
		End If
		rsget.Close

		If (((NOW() > orgpricestartDate AND NOW() < orgpriceendDate)) OR (NOW() > startDate AND NOW() < endDate)) AND mustCnt = 1 Then
			getSalePrice = Forgprice
		Else
			'2022-12-14 김진영 하단 주석처리
			getSalePrice = Clng(GetRaiseValue(MustPrice/10)*10)
...
'		isImageChanged = ibuf <> FRegImageName
	...
	Public Function getModelName
		...
		Else
			getModelName = "					<shop:ModelName>상품설명 참조</shop:ModelName>"
		End If
	End Function
...
'			buf = buf & "					<shop:Importer></shop:Importer>"							'수입사명 | 수입산인 경우 필수
			buf = buf & "					<shop:Plural>N</shop:Plural>"								'복수 원산지 | Y or N
			buf = buf & "					<shop:Content><![CDATA["& Chkiif(fnStrLength(Fsourcearea) >= 50, chrbyte(Fsourcearea,46,""), Fsourcearea) &"]]></shop:Content>"	'원산지 표시 내용 | Code가 "기타:직접 입력"인 경우 필수
		End If
		buf = buf & "				</shop:OriginArea>"
		getOriginAreaType = buf
	End Function

	Public Function getNvstorefarmCertInfo
		...
		If isRegCert = "Y" Then
			...
			If safetyDiv = "70" OR safetyDiv = "80" OR safetyDiv = "90" Then
				...
				buf = buf & "					<shop:CompanyName><![CDATA["&certmakerName&"]]></shop:CompanyName>"			'인증 상호 | 인증 유형이 방송통신기자재 적합인증/적합등록/잠정인증인 경우 필수
...
'			End If
			getNvstorefarmCertInfo = buf
		Else
			getNvstorefarmCertInfo = ""
		End If
	End Function

	Public Function getIsSafeChild
		Dim buf, strSql, safetyDiv, safetyId, certNum, certOrganName, certmakerName, isRegCert
		strSql = ""
	...
	Public Function getImageType()
		...
		basicimgStr	= ""
		strSql = ""
		strSql = strSql & " SELECT TOP 10 imgType, storefarmURL FROM db_etcmall.[dbo].[tbl_nvstorefarm_Image] WHERE itemid = '"&FItemid&"' "
		rsget.CursorLocation = adUseClient
		rsget.Open strSql, dbget, adOpenForwardOnly, adLockReadOnly
		If not rsget.Eof Then
...
'		buf = buf & "					<shop:IndividualCustomUniqueCode></shop:IndividualCustomUniqueCode>"	'개인통관 고유부호 수집 여부 Y or N | 미입력시 Y로 저장
	...
	If FItemdiv = "06" OR FItemdiv = "16" Then
		Dim mayDeliverDay, mayPeriodType
		...
		buf = buf & "					<shop:ExpectedDeliveryPeriodType>"&mayPeriodType&"</shop:ExpectedDeliveryPeriodType>"	'발송 예정일 타입 코드'
'		buf = buf & "					<shop:ExpectedDeliveryPeriodDirectInput>"&mayDeliverDay&"</shop:ExpectedDeliveryPeriodDirectInput>"	'발송 예정'일 직접 입력값
	...
	Public Function getNvstorefarmItemContParamToReg()
		...
		strRst = strRst & ("<p><center><a href=""http://storefarm.naver.com/tenbyten"" target=""_blank""><img src=""http://fiximage.10x10.co.kr/web2008/etc/top_notice_nvstorefarm.jpg""></a></center></p><br>")
		...
		'#배송 주의사항
		strRst = strRst & ("<br><img src=""http://fiximage.10x10.co.kr/web2008/etc/cs_info_nvstorefarm.jpg"">")
		strRst = strRst & ("</div>")
		getNvstorefarmItemContParamToReg = strRst
...
'		buf = buf & "				<shop:SellerCustomCode2></shop:SellerCustomCode2>"				'판매자가 내부에서 사용하는 코드
	...
	Public Function getNvstorefarmItemInfoCdToReg
		...
		If Not(rsget.EOF or rsget.BOF) then
			...
			Do until rsget.EOF
				...
				'품명 및 모델명 50자 넘으면 안되서 처리..2021-07-23 김진영
				If mallinfoCd = "ItemName" or mallinfoCd = "ModelName" Then
					infoContent = chrbyte(infoContent,46,"")
				End If
		...
		End If
		rsget.Close
		getNvstorefarmItemInfoCdToReg = buf
	End Function

	'// 업로드 이미지 XML 생성
	Public Function getNvstorefarmImageRegXML(oServ, oOper)
		...
		getNvstorefarmImageRegXML = strRst
	End Function

	'// 상품등록 XML 생성
	Public Function getNvstorefarmItemRegXML(oServ, oOper, isEdit)
		...
		If isEdit = "Y" Then
			strRst = strRst & "			<shop:ProductId>"&FNvstorefarmGoodNo&"</shop:ProductId>"		'상품ID | 없으면 등록, 있으면 수정
		End If
...
'		strRst = strRst & "				<shop:AttributeValueList></shop:AttributeValueList>"		' ,로 분리된 속성의 목록 | 현재는 사용하지 않으며 향후 사용 예정
		If FNeedCert = "Y" Then
			strRst = strRst & getNvstorefarmCertInfo()
		End If

		If FNeedCert = "Y" Then
			If getNvstorefarmCertInfo = "" Then
				strRst = strRst & "			<shop:ChildCertifiedProductExclusion>Y</shop:ChildCertifiedProductExclusion>"	'어린이상품 인증 대상 제외 여부 | “Y” 또는 “N” “어린이상품 인증 대상” 카테고리 상품에만 유효함. 미입력 시 “N”으로 저장된다.
...
'		strRst = strRst & "				<shop:ValidDate></shop:ValidDate>"							'유효 일자 | YYYY-MM-DD 형식
		...
		strRst = strRst & "				<shop:DetailContent><![CDATA["&getNvstorefarmItemContParamToReg&"]]></shop:DetailContent>"		'#상품 상세 정보
...
'		strRst = strRst & "				<shop:SalePrice>"&Clng(GetRaiseValue(MustPrice/10)*10)&"</shop:SalePrice>"		'#판매가
		...
		If (isEdit = "Y")  Then
			If (Foptioncnt = 0) Then
				strRst = strRst & "				<shop:StockQuantity>"&getLimitNvstorefarmEa&"</shop:StockQuantity>"		'#재고 수량 | 상품등록시 필수, 상품 수정시 재고 수량을 입력하지 않으면 스토어팜 DB에 저장된 현재 재고값이 변하지 않는다. 수정시 재고 수량 0으로 입력되면 StatusType으로 전달된 항목은 무시되며 상품 상태는 OSTK(품절)로 저장됨
			End If
		Else
			strRst = strRst & "				<shop:StockQuantity>"&getLimitNvstorefarmEa&"</shop:StockQuantity>"		'#재고 수량 | 상품등록시 필수, 상품 수정시 재고 수량을 입력하지 않으면 스토어팜 DB에 저장된 현재 재고값이 변하지 않는다. 수정시 재고 수량 0으로 입력되면 StatusType으로 전달된 항목은 무시되며 상품 상태는 OSTK(품절)로 저장됨
		End If
...
'		strRst = strRst & "				<shop:WifiOnly></shop:WifiOnly>"		'Wifi 전용 상품 여부 | Y or N..태블릿 카테고리 상품인 경우 필수..Y 입력시 PurchaseApplicationUrl, CellPhonePrice 입력불가..N 입력시 PurchaseApplicationUrl, CellPhonePrice 입력 필수
		...
		strRst = strRst & getNvstorefarmItemInfoCdToReg
		...
		strRst = strRst & "	</soapenv:Body>"
		strRst = strRst & "</soapenv:Envelope>"
		getNvstorefarmItemRegXML = strRst
	End Function
End Class
...
Class CNvstorefarm
	...
	Public Sub getNvstorefarmNotRegOneItem
		Dim strSql, addSql, i
		...
		strSql = strSql & "	, isNULL(R.nvstorefarmStatCD,-9) as nvstorefarmStatCD"
		...
		strSql = strSql & "	, isnull(bm.CateKey, '') as CateKey, isnull(R.APIaddImg, '') as APIaddImg "
		strSql = strSql & "	,isNull(v.itemid, 0) as nvstoremoonbanguid, p.purchasetype "
		strSql = strSql & " FROM db_item.dbo.tbl_item as i "
		...
		strSql = strSql & " LEFT JOIN db_etcmall.[dbo].[tbl_nvstorefarm_regItem] R on i.itemid = R.itemid"
		strSql = strSql & " LEFT JOIN db_etcmall.[dbo].[tbl_nvstoremoonbangu_regItem] v on i.itemid = v.itemid"
		...
		strSql = strSql & " and convert(varchar(6), i.cate_large + i.cate_mid) <> '035021' "	'여행/취미(035) > 애완용품(021)만 등록불가 // 2021-02-24 김진영
...
'		strSql = strSql & " and (i.sellcash <> 0 and ((i.sellcash - i.buycash)/i.sellcash)*100 >= " & CMAXMARGIN & ")"
		...
		''strSql = strSql & "	and i.makerid not in (Select makerid From db_temp.dbo.tbl_EpShop_not_in_makerid Where mallgubun='"&CMALLGUBUN&"' and isusing = 'N') "	'등록제외 브랜드
		''strSql = strSql & "	and i.itemid not in (Select itemid From db_temp.dbo.tbl_EpShop_not_in_itemid Where mallgubun='"&CMALLGUBUN&"' and isusing = 'Y') "		'등록제외 상품
		strSql = strSql & " and not exists(SELECT top 1 n.makerid FROM [db_temp].dbo.tbl_jaehyumall_not_in_makerid n with (nolock) WHERE n.makerid=i.makerid and n.mallgubun = 'nvstorefarm') "
		strSql = strSql & " and not exists(SELECT top 1 n.itemid FROM [db_temp].dbo.tbl_jaehyumall_not_in_itemid n with (nolock) WHERE n.itemid=i.itemid and n.mallgubun = 'nvstorefarm') "
		strSql = strSql & " and not exists(select top 1 y.idx FROM db_temp.[dbo].[tbl_jaehyumall_not_in_category] y with (nolock) "
		strSql = strSql & " 				WHERE convert(varchar(6), y.cdl + y.cdm) = convert(varchar(6), (i.cate_large + i.cate_mid)) and y.mallgubun = 'nvstorefarm') "
		If FRectGubun <> "IMG" Then
			strSql = strSql & "	and i.itemid not in (Select itemid From db_etcmall.[dbo].[tbl_nvstorefarm_regItem] where nvstorefarmStatCD > 3) "
		End If
		...
		If not rsget.EOF Then
			Set FOneItem = new CNvstorefarmItem
                ...
                FOneItem.FnvstorefarmStatCD	= rsget("nvstorefarmStatCD")
				...
				FOneItem.FNvstoremoonbanguid 	= rsget("nvstoremoonbanguid")
	...
	Public Sub getNvstorefarmEditOneItem
		Dim strSql, addSql, i
		If FRectItemID <> "" Then
		...
		strSql = ""
		strSql = strSql & " SELECT TOP " & FPageSize & " i.* "
		strSql = strSql & "	, c.keywords, c.ordercomment, c.sourcearea, c.makername, c.usingHTML, c.itemcontent, isNULL(c.requireMakeDay,0) as requireMakeDay, p.purchasetype "
		strSql = strSql & "	, isNULL(m.nvstorefarmGoodNo, '') as nvstorefarmGoodNo, m.nvstorefarmprice, m.nvstorefarmSellYn, isNULL(m.regedOptCnt, 0) as regedOptCnt "
		...
		strSql = strSql & "	, C.infoDiv, isNULL(C.safetyyn,'N') as safetyyn, isNULL(C.safetyDiv,0) as safetyDiv, C.safetyNum, isnull(ca.needCert, '') as needCert "
...
'		strSql = strSql & "		or ((i.sailyn = 'N') and ( ((i.sellcash-i.buycash)/i.sellcash)*100 < "&CMAXMARGIN&" )) "
		...
		strSql = strSql & " 	or exists(SELECT top 1 n.makerid FROM [db_temp].dbo.tbl_jaehyumall_not_in_makerid n with (nolock) WHERE n.makerid=i.makerid and n.mallgubun = 'nvstorefarm') "
		strSql = strSql & " 	or exists(SELECT top 1 n.itemid FROM [db_temp].dbo.tbl_jaehyumall_not_in_itemid n with (nolock) WHERE n.itemid=i.itemid and n.mallgubun = 'nvstorefarm') "
		...
		strSql = strSql & " JOIN db_item.dbo.tbl_item_contents as c on i.itemid = c.itemid "
		strSql = strSql & " JOIN db_partner.dbo.tbl_partner as p with (nolock) on i.makerid = p.id"
		strSql = strSql & " JOIN db_etcmall.dbo.tbl_nvstorefarm_regItem as m on i.itemid = m.itemid "
		strSql = strSql & " LEFT JOIN db_user.dbo.tbl_user_c uc on i.makerid = uc.userid"
		strSql = strSql & " LEFT JOIN db_etcmall.dbo.tbl_nvstorefarm_cate_mapping as bm on bm.tenCateLarge=i.cate_large and bm.tenCateMid=i.cate_mid and bm.tenCateSmall=i.cate_small "
		strSql = strSql & " LEFT JOIN db_etcmall.dbo.tbl_nvstorefarm_category as ca on ca.catekey = bm.catekey "
		strSql = strSql & " WHERE 1 = 1"
		strSql = strSql & " and m.APIaddImg = 'Y' "
		strSql = strSql & " and m.nvstorefarmStatCD = 7 "
		strSql = strSql & addSql
		strSql = strSql & " and m.nvstorefarmGoodNo is Not Null "									'#등록 상품만
		rsget.CursorLocation = adUseClient
		rsget.Open strSql, dbget, adOpenForwardOnly, adLockReadOnly
		FResultCount = rsget.RecordCount
		FTotalCount = FResultCount
		If not rsget.EOF Then
			Set FOneItem = new CnvstorefarmItem
				...
				FOneItem.FUsingHTML			= rsget("usingHTML")
				FOneItem.Fitemcontent		= db2html(rsget("itemcontent"))
				FOneItem.FNvstorefarmGoodNo		= rsget("nvstorefarmGoodNo")
				FOneItem.FNvstorefarmprice		= rsget("nvstorefarmprice")
				FOneItem.FnvstorefarmSellYn		= rsget("nvstorefarmSellYn")
...
Function getNvstorefarmGoodNo(iitemid)
	Dim strSql
	strSql = ""
	strSql = strSql & " SELECT TOP 1 nvstorefarmGoodNo FROM db_etcmall.[dbo].[tbl_nvstorefarm_regItem] WHERE itemid = '"&iitemid&"' "
	rsget.CursorLocation = adUseClient
	rsget.Open strSql, dbget, adOpenForwardOnly, adLockReadOnly
		getNvstorefarmGoodNo = rsget("nvstorefarmGoodNo")
	rsget.Close
End Function
