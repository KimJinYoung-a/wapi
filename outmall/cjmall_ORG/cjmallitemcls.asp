...
Class CCjmall
	...
	Public Sub getTencjmallCateList
		...
		sqlStr = sqlStr & " 	JOIN db_etcmall.dbo.tbl_cjMall_Category as cc on cc.CateKey = cm.CateKey  "  & VBCRLF
...
'		End If
		...
		sqlStr = sqlStr & " JOIN db_AppWish.dbo.tbl_item_contents as ct on i.itemid = ct.itemid"
		If (FRectIsReged = "N") OR (FRectIsReged = "A") Then		'//미등록이 아니면 JOIN
			sqlStr = sqlStr & "	LEFT JOIN db_outmall.dbo.tbl_cjmall_regitem as J on J.itemid = i.itemid "
		Else
			sqlStr = sqlStr & "	JOIN db_outmall.dbo.tbl_cjmall_regitem as J on J.itemid = i.itemid "
		End If
		sqlStr = sqlStr & "	LEFT JOIN db_outmall.dbo.tbl_OutMall_CateMap_Summary as c on c.mallid='"&CMALLNAME&"' and c.tenCateLarge = i.cate_large and c.tenCateMid = i.cate_mid and c.tenCateSmall = i.cate_small "
		sqlStr = sqlStr & " LEFT JOIN db_AppWish.dbo.tbl_user_c uc on i.makerid = uc.userid "
		sqlStr = sqlStr & " LEFT JOIN db_outmall.dbo.tbl_cjMall_prdDiv_mapping as PD on PD.tencatelarge = i.cate_large and PD.tencatemid = i.cate_mid and PD.tencatesmall = i.cate_small and ct.infodiv = PD.infodiv "
		...
		If (FRectIsReged <> "N" and FRectExtNotReg <> "Q")  Then		'// 미등록도 아니고 등록실패도 아니면 조건 없음
			If FRectIsReged = "Q" Then 
				...
				sqlStr = sqlStr & " or i.makerid  in (Select makerid From [db_outmall].dbo.tbl_jaehyumall_not_in_makerid Where mallgubun='"&CMALLNAME&"') "
				sqlStr = sqlStr & " or i.itemid  in (Select itemid From [db_outmall].dbo.tbl_jaehyumall_not_in_itemid Where mallgubun='"&CMALLNAME&"') "
		...
		sqlStr = sqlStr & "	, J.cjmallRegdate, J.cjmallLastUpdate, J.cjmallPrdNo, J.cjmallPrice, J.cjmallSellYn, J.regUserid, IsNULL(J.cjmallStatCd,-9) as cjmallStatCd "
		sqlStr = sqlStr & "	, J.regedOptCnt, J.rctSellCNT, J.accFailCNT, J.lastErrStr, PD.infodiv, PD.cdmKey, PD.cddkey, UC.defaultfreeBeasongLimit, i.itemdiv "
		sqlStr = sqlStr & " FROM db_AppWish.dbo.tbl_item as i "
		sqlStr = sqlStr & " JOIN db_AppWish.dbo.tbl_item_contents as ct on i.itemid = ct.itemid"
		If (FRectIsReged = "N") OR (FRectIsReged = "A") Then		'//미등록이 아니면 JOIN
			sqlStr = sqlStr & "	LEFT JOIN db_outmall.dbo.tbl_cjmall_regitem as J on J.itemid = i.itemid "
...
'			    addSql = addSql & " and J.itemid is NULL  and (i.limityn='N' or (i.limityn='Y' and i.limitno-i.limitsold>5)) "
		...
		sqlStr = sqlStr & " LEFT JOIN db_outmall.dbo.tbl_cjMall_prdDiv_mapping as PD on PD.tencatelarge = i.cate_large and PD.tencatemid = i.cate_mid and PD.tencatesmall = i.cate_small and t.infodiv = PD.infodiv "
		sqlStr = sqlStr & " WHERE 1 = 1 " & VBCRLF
