...
Dim i, chgSellYn
Dim retFlag
Dim sqlStr, AssignedRow, SubNodes
...
If cmdparam = "DELETE" Then
	...
	sqlStr = sqlStr &" SELECT 'ssg', i.itemid, r.ssgGoodNo, r.ssgRegdate, getdate(), r.lastErrStr " & VBCRLF
	sqlStr = sqlStr &" FROM db_item.dbo.tbl_item as i " & VBCRLF
	sqlStr = sqlStr &" JOIN db_etcmall.dbo.tbl_ssg_regitem as r on i.itemid = r.itemid " & VBCRLF
	sqlStr = sqlStr &" WHERE i.itemid in (" & arrItemid & ") "
	dbget.Execute(sqlStr)

	sqlStr = ""
	sqlStr = sqlStr & " delete from db_etcmall.dbo.tbl_ssg_regitem where itemid in (" & arrItemid & ")" & VbCrlf
	dbget.Execute sqlStr,AssignedRow
	rw AssignedRow&"건 상품 삭제"

	sqlStr = ""
	sqlStr = sqlStr & " delete from db_item.dbo.tbl_OutMall_regedoption where itemid in (" & arrItemid & ") and mallid = 'ssg' "
	dbget.Execute sqlStr,AssignedRow
	rw AssignedRow&"건 옵션 삭제"

	sqlStr = ""
	sqlStr = sqlStr & " delete from db_etcmall.dbo.tbl_outmall_API_Que where itemid in (" & arrItemid & ") and mallid = 'ssg' "
	dbget.Execute sqlStr,AssignedRow
...
<script type="text/javascript">
	...
	function arrSubmit(ino){
        ...
        document.frmSvArr.action = '/outmall/ssg/ssgActProc.asp';
...
<div id="actStr"></div>
<iframe name="xLink2" id="xLink2" frameborder="0" width="100%" height="500"></iframe>
