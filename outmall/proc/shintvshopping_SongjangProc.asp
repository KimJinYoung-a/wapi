...
If mode = "updateSendState" Then
	...
	sqlStr = sqlStr & "	and sellsite='shintvshopping'"
...
'/////////////////////////////////////
istrParam = ""
istrParam = istrParam & "linkCode=" & linkCode						'연결코드
istrParam = istrParam & "&entpCode=" & entpCode						'업체코드
istrParam = istrParam & "&entpId=" & entpId							'업체사용자ID
istrParam = istrParam & "&entpPass=" & entpPass						'업체PASSWORD
istrParam = istrParam & "&orderGb=10"								'배송 구분 값 | 10:주문, 40:교환
...
istrParam = istrParam & "&slipINo=" & beasongNum					'운송장식별번호
istrParam = istrParam & "&slipNo=" & inv_no							'운송장 번호
istrParam = istrParam & "&delyGb=" & deliveryCompanyCode			'배송사 코드
IsSuccss = false
Set objXML= CreateObject("MSXML2.ServerXMLHTTP.3.0")
	objXML.open "POST", shintvshoppingAPIURL & "/partner/delivery/delivery-out", false
...
if (IsSuccss) then
	...
	If errCount > 0 Then
		...
		response.write "    var uri = 'shintvshopping_SongjangProc.asp?mode=updateSendState&ord_no='+ORG_ord_no+'&ord_dtl_sn='+ord_dtl_sn+'&beasongNum='+beasongNum+'&updateSendState='+selectValue;"&VbCRLF
