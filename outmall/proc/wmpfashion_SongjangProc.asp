...
If mode = "updateSendState" Then
	...
	sqlStr = sqlStr & "	and sellsite='wmpfashion'"
...
IF (InStr(ord_no,"_")>0) then
	ord_no = getOutmallRefOrgOrderNO(ord_no,ord_dtl_sn,"wmpfashion")
end if
...
Set objXML= CreateObject("MSXML2.ServerXMLHTTP.3.0")
	...
	Else
		objXML.open "POST", "http://110.93.128.100:8090/fwmp/Orders/outputproc", false
	End If
	objXML.setRequestHeader "Content-Type", "application/x-www-form-urlencoded"
	objXML.Send(istrParam)
...
If (INSTR(iMessage, "주문확인건만 발송처리 가능합니다") > 0) Then
	ret1 = fnWMPfashionConfirmOrder(ord_no)
	If (ret1) then
		On Error Resume Next
		Set objXML= CreateObject("MSXML2.ServerXMLHTTP.3.0")
			If application("Svr_Info") = "Dev" Then
...
'	strSql = strSql & "	and OrgDetailKey='"&ord_dtl_sn&"'"&VBCRLF
	...
	If errCount > 0 Then
		...
		response.write "    var uri = 'wmpfashion_SongjangProc.asp?mode=updateSendState&ord_no='+ORG_ord_no+'&updateSendState='+selectValue;"&VbCRLF
