...
<body bgcolor="#F4F4F4" >
<%
Function getGmarketDlvFinXMLStr(masterno, detailno, delicompCd, wbNo, isDiv)
'delicompCd : 주문번호
'wbNo		: 송장
'delicompCd	: 택배코드
'rw isDiv
'response.end
	...
	strRst = strRst & "		<AddShippingComplete xmlns=""http://tpl.gmarket.co.kr/"">"
	strRst = strRst & "			<AddShippingComplete PackNo="""&masterno&""" ContrNo="""&detailno&""" ExpressName="""&delicompCd&""" InvoiceNo="""&wbNo&""" ShippingDate="""&Date()&""" />"
	strRst = strRst & "		</AddShippingComplete>"
	strRst = strRst & "	</soap:Body>"
	strRst = strRst & "</soap:Envelope>"
	getGmarketDlvFinXMLStr = strRst
End function
...
'###############################################################################################################################################################
Dim strSql, actCnt
Dim AssignedCNT, objXML, retCode, iMessage, ContrNo, xmlDOM
...
Dim xmlStr : xmlStr = getGmarketDlvFinXMLStr(ord_no, ord_dtl_sn, hdc_cd, inv_no, s_Div)
Dim retDoc, sURL
...
Set objXML = Server.CreateObject("MSXML2.ServerXMLHTTP.3.0")
	...
	objXML.setRequestHeader "SOAPAction", "http://tpl.gmarket.co.kr/AddShippingComplete"
	objXML.send(xmlStr)
...
'			response.end
	'		On Error Resume Next
			If xmlDOM.selectSingleNode("Envelope/Body").firstChild.nodeName = "AddShippingCompleteResponse" Then
				ContrNo = xmlDOM.getElementsByTagName("AddShippingCompleteResult ").item(0).getAttribute("ContrNo")
				retCode = xmlDOM.getElementsByTagName("AddShippingCompleteResult ").item(0).getAttribute("Result")
				iMessage = xmlDOM.getElementsByTagName("AddShippingCompleteResult ").item(0).getAttribute("Comment")
			End IF
	'		On Error Goto 0
		Set xmlDOM = nothing
'	End If
