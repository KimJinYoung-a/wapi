...
<%
Function fnWMPfashionConfirmOrder(vOrderserial)
	...
	Set objXML= CreateObject("MSXML2.ServerXMLHTTP.3.0")
		...
		Else
			objXML.open "POST", "http://110.93.128.100:8090/fwmp/Orders/ordercomplete", false
		End If
...
'			response.write iRbody
			Set strObj = JSON.parse(iRbody)
				...
				If isSuccessCode = "200" Then
					...
					strSql = strSql & " and mallid = 'wmpfashion' "
					dbget.Execute strSql
					fnWMPfashionConfirmOrder= true
				End If
			Set strObj = nothing
		End If
	Set objXML = nothing
...
Call GetCheckStatus("wmpfashion", iSellDate, iIsSuccess)
'iSellDate = "2019-01-28"
'rw iSellDate
If sellsite = "wmpfashion" Then
	On Error Resume Next
	Set objXML= CreateObject("MSXML2.ServerXMLHTTP.3.0")
		If application("Svr_Info") = "Dev" Then
			objXML.open "GET", "http://localhost:62569/Wemake/Orders/orderlist?reqdate="&iSellDate&"&type=NEW", false
		Else
			objXML.open "GET", "http://110.93.128.100:8090/fwmp/Orders/orderlist?reqdate="&iSellDate&"&type=NEW", false
		End If
...
' rw "deliverymemo : " & deliverymemo
							set obj2 = obj1.get(i).orderProduct
								For j=0 to obj2.length-1
									...
									SellPrice			= obj2.get(j).productPrice		'상품금액
									...
									set obj3 = obj2.get(j).orderOption
										For k=0 to obj3.length-1
											...
											If (retVal) Then
												...
												strsql = strsql & " 		VALUES ('"&OutMallOrderSerial&"', '"&OrgDetailKey&"', '', 'N', getdate(), 'wmpfashion')"
												strSql = strSql & " 	END "
												dbget.Execute strSql
											Else
												failCNT = failCNT + 1
			...
			If (succCNT > 0) then
				...
				strsql = strsql & " and T.mallid = 'wmpfashion' "
				dbget.Execute strsql
				...
				For lp = 0 To Ubound(arrList, 2)
					ret1 = fnWMPfashionConfirmOrder(arrList(0, lp))
