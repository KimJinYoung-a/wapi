...
Dim grpVal : grpVal = request("grpVal")
Dim rSkip : rSkip = request("rSkip")
Dim rLimit : rLimit = request("rLimit")
Dim i, strParam, iErrStr, ret1
Dim sqlStr, strSql, AssignedRow, SubNodes
Dim chgSellYn, actCnt, retErrStr
Dim buf, buf2, iitemid
Dim ArrRows
Dim retFlag
Dim iMessage
...
If cmdparam = "DELETE" Then
	...
	sqlStr = sqlStr &" SELECT 'lotteon', i.itemid, r.lotteonGoodNo, r.lotteonRegdate, getdate(), r.lastErrStr " & VBCRLF
	sqlStr = sqlStr &" FROM db_item.dbo.tbl_item as i " & VBCRLF
	sqlStr = sqlStr &" JOIN db_etcmall.dbo.tbl_lotteon_regitem as r on i.itemid = r.itemid " & VBCRLF
	sqlStr = sqlStr &" WHERE i.itemid in (" & arrItemid & ") "
	dbget.Execute(sqlStr)

	sqlStr = ""
	sqlStr = sqlStr & " delete from db_etcmall.dbo.tbl_lotteon_regitem where itemid in (" & arrItemid & ")" & VbCrlf
	dbget.Execute sqlStr,AssignedRow
	rw AssignedRow&"건 상품 삭제"

	sqlStr = ""
	sqlStr = sqlStr & " delete from db_item.dbo.tbl_OutMall_regedoption where itemid in (" & arrItemid & ") and mallid = 'lotteon' "
	dbget.Execute sqlStr,AssignedRow
	rw AssignedRow&"건 옵션 삭제"

	sqlStr = ""
	sqlStr = sqlStr & " delete from db_etcmall.dbo.tbl_outmall_API_Que where itemid in (" & arrItemid & ") and mallid = 'lotteon' "
	dbget.Execute sqlStr,AssignedRow
...
<script type="text/javascript">
	...
	function arrSubmit(ino){
		...
		document.frmSvArr.grpVal.value = "<%=grpVal%>";
		document.frmSvArr.rSkip.value = "<%=rSkip%>";
		document.frmSvArr.rLimit.value = "<%=rLimit%>";
        document.frmSvArr.action = '/outmall/lotteon/lotteonActProc.asp';
...
<form name="frmSvArr">
	...
	<input type="hidden" name="grpVal">
	<input type="hidden" name="rSkip">
	<input type="hidden" name="rLimit">
</form>
