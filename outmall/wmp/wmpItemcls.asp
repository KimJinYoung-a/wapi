...
'		strSql = strSql & " and (i.sellcash<>0 and convert(int, ((i.sellcash-i.buycash)/i.sellcash)*100)>=" & CMAXMARGIN & ")"		''2019-02-28 18:10 김진영..주석처리
		strSql = strSql & " and (i.sellcash <> 0) "
	...
	Public Sub getWmpEditSaleOneItem
		...
		strSql = strSql & addSql
		strSql = strSql & " and m.wemakeGoodNo is Not Null "									'#등록 상품만
		rsget.CursorLocation = adUseClient
		rsget.Open strSql, dbget, adOpenForwardOnly, adLockReadOnly
		FResultCount = rsget.RecordCount
		FTotalCount = FResultCount
