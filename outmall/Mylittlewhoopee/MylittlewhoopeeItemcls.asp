...
Class CMylittlewhoopeeItem
	...
	Public FUsingHTML
	Public Fitemcontent
	Public FMylittlewhoopeeGoodNo
	Public FMylittlewhoopeeprice
	Public FMylittlewhoopeeSellYn
	Public FregedOptCnt
	Public FAccFailCNT
	Public FMaySoldOut
	Public Fregitemname
...
'		sqlStr = sqlStr & " and getdate() >= startDate and getdate() <= endDate "
	...
	'// 스토어팜 판매여부 반환
	Public Function getMylittlewhoopeeSellYn()
		'판매상태 (10:판매진행, 20:품절)
		If FSellYn="Y" and FIsUsing="Y" then
			If FLimitYn = "N" or (FLimitYn = "Y" and FLimitNo - FLimitSold >= CMAXLIMITSELL) then
				getMylittlewhoopeeSellYn = "Y"
			Else
				getMylittlewhoopeeSellYn = "N"
			End If
		Else
			getMylittlewhoopeeSellYn = "N"
		End If
	...
	Public Function getLimitMylittlewhoopeeEa()
		Dim ret
		If FLimitYn = "Y" Then
...
'			buf = buf & "					<shop:Importer></shop:Importer>"							'수입사명 | 수입산인 경우 필수
	...
	Public Function getMylittlewhoopeeCertInfo
...
'			End If
			getMylittlewhoopeeCertInfo = buf
		Else
			getMylittlewhoopeeCertInfo = ""
		End If
	End Function
	...
	Public Function getImageType()
		...
		basicimgStr	= ""
		strSql = ""
		strSql = strSql & " SELECT TOP 10 imgType, storefarmURL FROM db_etcmall.[dbo].[tbl_Mylittlewhoopee_Image] WHERE itemid = '"&FItemid&"' "
		rsget.CursorLocation = adUseClient
		rsget.Open strSql, dbget, adOpenForwardOnly, adLockReadOnly
		If not rsget.Eof Then
...
'		buf = buf & "					<shop:ExpectedDeliveryPeriodDirectInput>"&mayDeliverDay&"</shop:ExpectedDeliveryPeriodDirectInput>"	'발송 예정'일 직접 입력값
	...
	Public Function getMylittlewhoopeeItemContParamToReg()
		...
		strRst = strRst & ("</div>")
		getMylittlewhoopeeItemContParamToReg = strRst
...
'		buf = buf & "				<shop:SellerCustomCode2></shop:SellerCustomCode2>"				'판매자가 내부에서 사용하는 코드
	...
	Public Function getMylittlewhoopeeItemInfoCdToReg
...
'			    End If
		...
		End If
		rsget.Close
		getMylittlewhoopeeItemInfoCdToReg = buf
	End Function

	'// 업로드 이미지 XML 생성
	Public Function getMylittlewhoopeeImageRegXML(oServ, oOper)
		...
		getMylittlewhoopeeImageRegXML = strRst
	End Function

	'// 상품등록 XML 생성
	Public Function getMylittlewhoopeeItemRegXML(oServ, oOper, isEdit)
		...
		If isEdit = "Y" Then
			strRst = strRst & "			<shop:ProductId>"&FMylittlewhoopeeGoodNo&"</shop:ProductId>"		'상품ID | 없으면 등록, 있으면 수정
		End If
...
'		strRst = strRst & "				<shop:AttributeValueList></shop:AttributeValueList>"		' ,로 분리된 속성의 목록 | 현재는 사용하지 않으며 향후 사용 예정
		If FNeedCert = "Y" Then
			strRst = strRst & getMylittlewhoopeeCertInfo()
		End If
		If FNeedCert = "Y" Then
			If getMylittlewhoopeeCertInfo = "" Then
...
'		strRst = strRst & "				<shop:ValidDate></shop:ValidDate>"							'유효 일자 | YYYY-MM-DD 형식
		...
		strRst = strRst & "				<shop:DetailContent><![CDATA["&getMylittlewhoopeeItemContParamToReg&"]]></shop:DetailContent>"		'#상품 상세 정보
...
'		strRst = strRst & "				<shop:SalePrice>"&Clng(GetRaiseValue(MustPrice/10)*10)&"</shop:SalePrice>"		'#판매가
		...
		If (isEdit = "Y")  Then
			If (Foptioncnt = 0) Then
				strRst = strRst & "				<shop:StockQuantity>"&getLimitMylittlewhoopeeEa&"</shop:StockQuantity>"		'#재고 수량 | 상품등록시 필수, 상품 수정시 재고 수량을 입력하지 않으면 스토어팜 DB에 저장된 현재 재고값이 변하지 않는다. 수정시 재고 수량 0으로 입력되면 StatusType으로 전달된 항목은 무시되며 상품 상태는 OSTK(품절)로 저장됨
			End If
		Else
			strRst = strRst & "				<shop:StockQuantity>"&getLimitMylittlewhoopeeEa&"</shop:StockQuantity>"		'#재고 수량 | 상품등록시 필수, 상품 수정시 재고 수량을 입력하지 않으면 스토어팜 DB에 저장된 현재 재고값이 변하지 않는다. 수정시 재고 수량 0으로 입력되면 StatusType으로 전달된 항목은 무시되며 상품 상태는 OSTK(품절)로 저장됨
		End If
...
'		strRst = strRst & "				<shop:WifiOnly></shop:WifiOnly>"		'Wifi 전용 상품 여부 | Y or N..태블릿 카테고리 상품인 경우 필수..Y 입력시 PurchaseApplicationUrl, CellPhonePrice 입력불가..N 입력시 PurchaseApplicationUrl, CellPhonePrice 입력 필수
		...
		strRst = strRst & getMylittlewhoopeeItemInfoCdToReg
		...
		strRst = strRst & "	</soapenv:Body>"
		strRst = strRst & "</soapenv:Envelope>"
		getMylittlewhoopeeItemRegXML = strRst
	End Function
End Class

Class CMylittlewhoopee
	Public FOneItem
	Public FItemList()
	...
	Public Sub getMylittlewhoopeeNotRegOneItem
		Dim strSql, addSql, i
		...
		strSql = strSql & "	, isNULL(R.MylittlewhoopeeStatCD,-9) as MylittlewhoopeeStatCD"
		...
		strSql = strSql & " LEFT JOIN db_etcmall.[dbo].[tbl_Mylittlewhoopee_regItem] R on i.itemid = R.itemid"
		...
		strSql = strSql & " and not exists(SELECT top 1 n.makerid FROM [db_temp].dbo.tbl_jaehyumall_not_in_makerid n with (nolock) WHERE n.makerid=i.makerid and n.mallgubun = 'Mylittlewhoopee') "
		strSql = strSql & " and not exists(SELECT top 1 n.itemid FROM [db_temp].dbo.tbl_jaehyumall_not_in_itemid n with (nolock) WHERE n.itemid=i.itemid and n.mallgubun = 'Mylittlewhoopee') "
		strSql = strSql & " and not exists(select top 1 y.idx FROM db_temp.[dbo].[tbl_jaehyumall_not_in_category] y with (nolock) "
		strSql = strSql & " 				WHERE convert(varchar(6), y.cdl + y.cdm) = convert(varchar(6), (i.cate_large + i.cate_mid)) and y.mallgubun = 'Mylittlewhoopee') "
		If FRectGubun <> "IMG" Then
			strSql = strSql & "	and i.itemid not in (Select itemid From db_etcmall.[dbo].[tbl_Mylittlewhoopee_regItem] where MylittlewhoopeeStatCD > 3) "
		End If
		...
		If not rsget.EOF Then
			Set FOneItem = new CMylittlewhoopeeItem
                ...
                FOneItem.FMylittlewhoopeeStatCD	= rsget("MylittlewhoopeeStatCD")
	...
	Public Sub getMylittlewhoopeeEditOneItem
		Dim strSql, addSql, i
		If FRectItemID <> "" Then
		...
		strSql = strSql & "	, isNULL(m.MylittlewhoopeeGoodNo, '') as MylittlewhoopeeGoodNo, m.Mylittlewhoopeeprice, m.MylittlewhoopeeSellYn, isNULL(m.regedOptCnt, 0) as regedOptCnt "
...
'		strSql = strSql & "		or ((i.sailyn = 'N') and ( ((i.sellcash-i.buycash)/i.sellcash)*100 < "&CMAXMARGIN&" )) "
		...
		strSql = strSql & " 	or exists(SELECT top 1 n.makerid FROM [db_temp].dbo.tbl_jaehyumall_not_in_makerid n with (nolock) WHERE n.makerid=i.makerid and n.mallgubun = 'Mylittlewhoopee') "
		strSql = strSql & " 	or exists(SELECT top 1 n.itemid FROM [db_temp].dbo.tbl_jaehyumall_not_in_itemid n with (nolock) WHERE n.itemid=i.itemid and n.mallgubun = 'Mylittlewhoopee') "
		...
		strSql = strSql & " JOIN db_item.dbo.tbl_item_contents as c on i.itemid = c.itemid "
		strSql = strSql & " JOIN db_partner.dbo.tbl_partner as p with (nolock) on i.makerid = p.id"
		strSql = strSql & " JOIN db_etcmall.dbo.tbl_Mylittlewhoopee_regItem as m on i.itemid = m.itemid "
		strSql = strSql & " LEFT JOIN db_user.dbo.tbl_user_c uc on i.makerid = uc.userid"
		strSql = strSql & " LEFT JOIN db_etcmall.dbo.tbl_nvstorefarm_cate_mapping as bm on bm.tenCateLarge=i.cate_large and bm.tenCateMid=i.cate_mid and bm.tenCateSmall=i.cate_small "
		strSql = strSql & " LEFT JOIN db_etcmall.dbo.tbl_nvstorefarm_category as ca on ca.catekey = bm.catekey "
		strSql = strSql & " WHERE 1 = 1"
		strSql = strSql & " and m.APIaddImg = 'Y' "
		strSql = strSql & " and m.MylittlewhoopeeStatCD = 7 "
		strSql = strSql & addSql
		strSql = strSql & " and m.MylittlewhoopeeGoodNo is Not Null "									'#등록 상품만
		rsget.CursorLocation = adUseClient
		rsget.Open strSql, dbget, adOpenForwardOnly, adLockReadOnly
		FResultCount = rsget.RecordCount
		FTotalCount = FResultCount
		If not rsget.EOF Then
			Set FOneItem = new CMylittlewhoopeeItem
				...
				FOneItem.FUsingHTML			= rsget("usingHTML")
				FOneItem.Fitemcontent		= db2html(rsget("itemcontent"))
				FOneItem.FMylittlewhoopeeGoodNo		= rsget("MylittlewhoopeeGoodNo")
				FOneItem.FMylittlewhoopeeprice		= rsget("Mylittlewhoopeeprice")
				FOneItem.FMylittlewhoopeeSellYn		= rsget("MylittlewhoopeeSellYn")
...
Function getMylittlewhoopeeGoodNo(iitemid)
	Dim strSql
	strSql = ""
	strSql = strSql & " SELECT TOP 1 MylittlewhoopeeGoodNo FROM db_etcmall.[dbo].[tbl_Mylittlewhoopee_regItem] WHERE itemid = '"&iitemid&"' "
	rsget.CursorLocation = adUseClient
	rsget.Open strSql, dbget, adOpenForwardOnly, adLockReadOnly
		getMylittlewhoopeeGoodNo = rsget("MylittlewhoopeeGoodNo")
	rsget.Close
End Function
