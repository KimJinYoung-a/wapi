...
'이미지 업로드
Public Function fnMylittlewhoopeeImageReg(iitemid, strParam, byRef iErrStr, ichgImageNm, iservice, ioperation)
	Dim objXML, xmlDOM, strSql, iMessage, MylittlewhoopeeURL, SubNodes, ResponseType, imglist
	Dim myURL, yourURL
	If (application("Svr_Info") = "Dev") Then
		MylittlewhoopeeURL = "http://sandbox.api.naver.com/ShopN/"&iservice
	Else
		MylittlewhoopeeURL = "http://ec.api.naver.com/ShopN/"&iservice
	End If

	On Error Resume Next
	Set objXML = Server.CreateObject("MSXML2.ServerXMLHTTP.3.0")
		objXML.Open "POST", MylittlewhoopeeURL, False
 		...
 		If objXML.Status = 200 Then
			Set xmlDOM = Server.CreateObject("MSXML.DOMDocument")
				...
				If ResponseType = "SUCCESS" Then
					...
					strSql = strSql & " db_etcmall.[dbo].[tbl_Mylittlewhoopee_Image] " & vbcrlf
					...
					For Each SubNodes in imglist
						...
						If InStr(myURL, "/basic/") OR InStr(myURL, "/nvadd1/") Then
							strSql = ""
							strSql = strSql & " INSERT INTO db_etcmall.[dbo].[tbl_Mylittlewhoopee_Image] (itemid, imgType, tenURL, storefarmURL) VALUES " & vbcrlf
					...
					Set imglist = nothing
					strSql = ""
					strSql = strSql & " UPDATE db_etcmall.[dbo].[tbl_Mylittlewhoopee_regItem] SET "
...
'상품 둥록
Public Function fnMylittlewhoopeeItemReg(iitemid, strParam, byRef iErrStr, iSellCash, iMylittlewhoopeeSellYn, ilimityn, ilimitno, ilimiysold, iitemname, iimageNm, iservice, ioperation, ichkXML)
	Dim objXML, xmlDOM, strSql, iMessage, MylittlewhoopeeURL, SubNodes, ResponseType, imglist
	Dim ProductId
	If (application("Svr_Info") = "Dev") Then
		MylittlewhoopeeURL = "http://sandbox.api.naver.com/ShopN/"&iservice
...
'				If ResponseType = "SUCCESS" Then
				...
				If ResponseType = "SUCCESS" AND ProductId <> "" Then
					...
					strSql = strSql & " SET MylittlewhoopeeGoodNo = '"&ProductId&"' " & vbcrlf
					strSql = strSql & " , MylittlewhoopeeLastUpdate = getdate() " & vbcrlf
					strSql = strSql & " , MylittlewhoopeePrice = " & iSellCash & vbcrlf
					strSql = strSql & " , accFailCnt = 0 " & vbcrlf
					strSql = strSql & " , MylittlewhoopeeRegdate = getdate() " & vbcrlf
					strSql = strSql & " , MylittlewhoopeeStatCd = 7 " & vbcrlf
					strSql = strSql & "	FROM db_etcmall.[dbo].[tbl_Mylittlewhoopee_regItem] R " & vbcrlf
...
'상품 수정
Public Function fnMylittlewhoopeeItemEDIT(iitemid, strParam, byRef iErrStr, iSellCash, iMylittlewhoopeeSellYn, ilimityn, ilimitno, ilimiysold, iitemname, ichgImageNm, iservice, ioperation)
	Dim objXML, xmlDOM, strSql, iMessage, MylittlewhoopeeURL, SubNodes, ResponseType, imglist
	Dim ProductId
	If (application("Svr_Info") = "Dev") Then
		MylittlewhoopeeURL = "http://sandbox.api.naver.com/ShopN/"&iservice
 		...
 		If objXML.Status = 200 Then
			Set xmlDOM = Server.CreateObject("MSXML.DOMDocument")
				...
				If ResponseType = "SUCCESS" Then
					...
					strSql = ""
					strSql = strSql & " UPDATE R " & vbcrlf
					strSql = strSql & " SET MylittlewhoopeeLastUpdate = getdate() " & vbcrlf
					strSql = strSql & " , MylittlewhoopeePrice = " & iSellCash & vbcrlf
					strSql = strSql & " , accFailCnt = 0 " & vbcrlf
...
End Function
'옵션 등록
Public Function fnMylittlewhoopeeOptionReg(iitemid, strParam, byRef iErrStr, iservice, ioperation)
	Dim objXML, xmlDOM, strSql, iMessage, MylittlewhoopeeURL, SubNodes, ResponseType, imglist
...
'상품 조회
Public Function fnMylittlewhoopeeItemSearch(iitemid, strParam, byRef iErrStr, iservice, ioperation)
	Dim objXML, xmlDOM, strSql, iMessage, MylittlewhoopeeURL, SubNodes, ResponseType, imglist
	Dim myURL, yourURL, statusType, nvRegitemname, MasterPrice, ProductId
	If (application("Svr_Info") = "Dev") Then
		MylittlewhoopeeURL = "http://sandbox.api.naver.com/ShopN/"&iservice
 		...
 		If objXML.Status = 200 Then
			Set xmlDOM = Server.CreateObject("MSXML.DOMDocument")
				...
				If ResponseType = "SUCCESS" Then
					...
					strSQL = ""
					strSQL = strSQL & " UPDATE R" & VbCRLF
					strSQL = strSQL & " SET MylittlewhoopeePrice = " & MasterPrice & VbCRLF
					strSQL = strSQL & " ,MylittlewhoopeeSellyn='"&statusType&"'" & VbCRLF
					strSQL = strSQL & " ,regitemname='"&html2db(nvRegitemname)&"'" & VbCRLF
					strSQL = strSQL & " ,lastStatCheckDate = getdate()" & VbCRLF
					strSQL = strSQL & " ,MylittlewhoopeeGoodNo = isNULL(R.MylittlewhoopeeGoodNo,'"&ProductId&"')"&VbCRLF
					strSQL = strSQL & " FROM db_etcmall.[dbo].[tbl_Mylittlewhoopee_regItem] R" & VbCRLF
					strSQL = strSQL & " where R.itemid="&iitemid & VbCRLF
					strSQL = strSQL & " and isNULL(MylittlewhoopeeGoodNo,'') in ('','"&ProductId&"')"&VbCRLF    ''중복등록된CaSE 대비
					strSQL = strSQL & " and (isNULL(MylittlewhoopeePrice,0)<>"&MasterPrice&"" & VbCRLF
					strSQL = strSQL & "     or isNULL(MylittlewhoopeeSellyn,'')<>'"&statusType&"'"& VbCRLF
					strSQL = strSQL & "     or isNULL(regitemname,'')<>'"&html2db(nvRegitemname)&"'"& VbCRLF
					strSQL = strSQL & "     or isNULL(MylittlewhoopeeGoodNo,'')<>'"&ProductId&"'"& VbCRLF
					strSQL = strSQL & " )"
				    dbget.Execute strSQL
...
'옵션 조회
Public Function fnMylittlewhoopeeOptionSearch(iitemid, strParam, byRef iErrStr, iservice, ioperation)
	Dim objXML, xmlDOM, strSQL, iMessage, MylittlewhoopeeURL, SubNodes, ResponseType, imglist
	Dim myURL, yourURL, statusType, nvRegitemname, MasterPrice, ProductId
...
'판매 상태 변경
Public Function fnMylittlewhoopeeSellyn(iitemid, ichgSellYn, strParam, byRef iErrStr, iservice, ioperation)
	Dim objXML, xmlDOM, strSql, iMessage, MylittlewhoopeeURL, SubNodes, ResponseType, imglist
	Dim myURL, yourURL
	If (application("Svr_Info") = "Dev") Then
		MylittlewhoopeeURL = "http://sandbox.api.naver.com/ShopN/"&iservice
...
'response.end
				...
				If ResponseType = "SUCCESS" Then
					strSql = ""
					strSql = strSql & " UPDATE db_etcmall.[dbo].[tbl_Mylittlewhoopee_regItem] " & VbCRLF
					strSql = strSql & " SET MylittlewhoopeeSellYn = '"&ichgSellYn&"'" & VbCRLF
					strSql = strSql & " ,MylittlewhoopeeLastUpdate = getdate()" & VbCRLF
					strSql = strSql & " ,accFailCNT=0" & VbCRLF
					strSql = strSql & " WHERE itemid = "&iitemid
					dbget.Execute(strSql)
				...
				Else
					...
					If InStr(iMessage, "품절 상품은 판매중 상태로만 변경할 수 있습니다") OR InStr(iMessage, "판매금지 상태의") Then
						strSql = ""
						strSql = strSql & " UPDATE db_etcmall.[dbo].[tbl_Mylittlewhoopee_regItem] " & VbCRLF
						strSql = strSql & " SET MylittlewhoopeeSellYn = 'N'" & VbCRLF
						strSql = strSql & " ,MylittlewhoopeeLastUpdate = getdate()" & VbCRLF
						strSql = strSql & " ,accFailCNT=0" & VbCRLF
						strSql = strSql & " WHERE itemid = "&iitemid
						dbget.Execute(strSql)
...
'상품 삭제
Public Function fnMylittlewhoopeeDelete(iitemid, strParam, byRef iErrStr, iservice, ioperation)
	Dim objXML, xmlDOM, strSql, iMessage, MylittlewhoopeeURL, SubNodes, ResponseType, imglist
	Dim myURL, yourURL
	If (application("Svr_Info") = "Dev") Then
		MylittlewhoopeeURL = "http://sandbox.api.naver.com/ShopN/"&iservice
 		...
 		If objXML.Status = 200 Then
			Set xmlDOM = Server.CreateObject("MSXML.DOMDocument")
				...
				If ResponseType = "SUCCESS" Then
					...
					strSql = strSql &" SELECT TOP 1 'Mylittlewhoopee', i.itemid, r.MylittlewhoopeeGoodNo, r.Mylittlewhoopeeregdate, getdate(), r.lastErrStr" & VBCRLF
					strSql = strSql &" FROM db_item.dbo.tbl_item as i " & VBCRLF
					strSql = strSql &" JOIN db_etcmall.dbo.tbl_Mylittlewhoopee_regItem as r on i.itemid = r.itemid " & VBCRLF
					strSql = strSql &" WHERE i.itemid = "&iitemid & VBCRLF
					dbget.Execute(strSql)

					strSql = ""
					strSql = strSql & " DELETE FROM [db_etcmall].[dbo].[tbl_Mylittlewhoopee_regItem] " & vbcrlf
					strSql = strSql & " WHERE itemid = '"&iitemid&"' "
					dbget.Execute(strSql)

					strSql = ""
					strSql = strSql & " DELETE FROM [db_etcmall].[dbo].[tbl_Mylittlewhoopee_Image] " & vbcrlf
					strSql = strSql & " WHERE itemid = '"&iitemid&"' "
					dbget.Execute(strSql)
...
End Function
'상품 상태 변경 XML
Public Function getMylittlewhoopeeSellynParameter(iMylittlewhoopeegoodno, ichgSellYn, iservice, ioperation)
	...
	strRst = strRst &"				<shop:ProductId>"&iMylittlewhoopeegoodno&"</shop:ProductId>"
	strRst = strRst &"				<shop:StatusType>"&stopYN&"</shop:StatusType>"
	strRst = strRst &"			</SaleStatus>"
	strRst = strRst &"		</shop:ChangeProductSaleStatusRequest>"
	strRst = strRst &"	</soapenv:Body>"
	strRst = strRst &"</soapenv:Envelope>"
	getMylittlewhoopeeSellynParameter = strRst
End Function

'상품 옵션 등록 XML
Public Function getMylittlewhoopeeOptionRegXML(iitemid, iMylittlewhoopeegoodno, iservice, ioperation)
...
'					End If
		...
		strRst = strRst &"	</soapenv:Body>"
		strRst = strRst &"</soapenv:Envelope>"
		getMylittlewhoopeeOptionRegXML = strRst
	Else
		Dim isRegedOptCnt
		strSql = ""
		strSql = strSql &"  SELECT TOP 1 isnull(regedOptcnt, 0) as regedOptcnt "
		strSql = strSql & " FROM db_etcmall.[dbo].[tbl_Mylittlewhoopee_regItem]"
		strSql = strSql & " WHERE MylittlewhoopeeStatcd = 7 and itemid=" & iitemid
		rsget.CursorLocation = adUseClient
		rsget.Open strSql, dbget, adOpenForwardOnly, adLockReadOnly
		If Not(rsget.EOF or rsget.BOF) then
		...
		If (ioptioncnt = 0) and (isRegedOptCnt > 0) Then
			...
			getMylittlewhoopeeOptionRegXML = strRst
		Else
			getMylittlewhoopeeOptionRegXML = "X"
		End If
	End If
End Function

'상품 조회 XML
Public Function getMylittlewhoopeeItemSearchParameter(iMylittlewhoopeegoodno, iservice, ioperation)
	...
	strRst = strRst &"			<shop:Version>2.0</shop:Version>"
	strRst = strRst &"			<SellerId>"&reqID&"</SellerId>"
	strRst = strRst &"			<ProductId>"&iMylittlewhoopeegoodno&"</ProductId>"
	strRst = strRst &"		</shop:GetProductRequest>"
	strRst = strRst &"	</soapenv:Body>"
	strRst = strRst &"</soapenv:Envelope>"
	getMylittlewhoopeeItemSearchParameter = strRst
End Function

'옵션 조회 XML
Public Function getMylittlewhoopeeOptionSearchParameter(iMylittlewhoopeegoodno, iservice, ioperation)
	Dim strRst, oaccessLicense, oTimestamp, osignature, reqID
	If (application("Svr_Info") = "Dev") Then
		reqID = "qa2tc329"
	Else
	...
	strRst = strRst &"	</soapenv:Body>"
	strRst = strRst &"</soapenv:Envelope>"
	getMylittlewhoopeeOptionSearchParameter = strRst
End Function
...
'상품 삭제 XML
Public Function getMylittlewhoopeeDeleteParameter(iMylittlewhoopeegoodno, iservice, ioperation)
	Dim strRst, oaccessLicense, oTimestamp, osignature, reqID
	If (application("Svr_Info") = "Dev") Then
		reqID = "qa2tc329"
	Else
	...
	strRst = strRst &"	</soapenv:Body>"
	strRst = strRst &"</soapenv:Envelope>"
	getMylittlewhoopeeDeleteParameter = strRst
End Function
...
Public Function getAddressBookList(iccd)
	...
	strRst = strRst & "   </soapenv:Body>"
	strRst = strRst & "</soapenv:Envelope>"
	Dim MylittlewhoopeeURL
	If (application("Svr_Info") = "Dev") Then
		MylittlewhoopeeURL = "http://sandbox.api.naver.com/ShopN/"&oServ
	Else
		MylittlewhoopeeURL = "http://ec.api.naver.com/ShopN/"&oServ
	End If

	Dim httpRequest, xmlDOM
	Set httpRequest = CreateObject("MSXML2.XMLHTTP")

	httpRequest.Open "POST", MylittlewhoopeeURL, False
				...
				If xmlDOM.getElementsByTagName("n:Category").length > 0 Then
					Set Nodes = xmlDOM.getElementsByTagName("n:Category")
						For each SubNodes in Nodes
							...
							strSql = ""
							strSql =  strSql & " IF Exists(SELECT * FROM db_etcmall.dbo.tbl_Mylittlewhoopee_category WHERE catekey = '"&categoryId&"') "
							strSql =  strSql & " 	BEGIN "
							strSql =  strSql & " 		UPDATE db_etcmall.dbo.tbl_Mylittlewhoopee_category SET "
							...
							strSql =  strSql & " ELSE "
							strSql =  strSql & " 	BEGIN "
							strSql =  strSql & " 		INSERT INTO db_etcmall.dbo.tbl_Mylittlewhoopee_category "
				...
				If xmlDOM.getElementsByTagName("n:ExceptionalCategoryList").length > 0 Then
					Set Nodes = xmlDOM.getElementsByTagName("n:ExceptionalCategory")
						For each SubNodes in Nodes
							...
							strSql = ""
							strSql =  strSql & " INSERT INTO db_etcmall.[dbo].[tbl_Mylittlewhoopee_certInfo] (CateKey, certCode, certName, lastUpdate) "
