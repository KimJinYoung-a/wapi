...
If cmdparam = "DELETE" Then
	...
	sqlStr = sqlStr &" SELECT '11st1010', i.itemid, r.st11GoodNo, isNull(r.st11Regdate, getdate()), getdate(), r.lastErrStr " & VBCRLF
	sqlStr = sqlStr &" FROM db_item.dbo.tbl_item as i " & VBCRLF
	sqlStr = sqlStr &" JOIN db_etcmall.dbo.tbl_11st_regitem as r on i.itemid = r.itemid " & VBCRLF
	sqlStr = sqlStr &" WHERE i.itemid in (" & arrItemid & ") "
	dbget.Execute(sqlStr)

	sqlStr = ""
	sqlStr = sqlStr & " delete from db_etcmall.dbo.tbl_11st_regitem where itemid in (" & arrItemid & ")" & VbCrlf
	dbget.Execute sqlStr,AssignedRow
	rw AssignedRow&"건 상품 삭제"

	sqlStr = ""
	sqlStr = sqlStr & " delete from db_item.dbo.tbl_OutMall_regedoption where itemid in (" & arrItemid & ") and mallid = '11st1010' "
	dbget.Execute sqlStr,AssignedRow
	rw AssignedRow&"건 옵션 삭제"

	sqlStr = ""
	sqlStr = sqlStr & " delete from db_etcmall.dbo.tbl_outmall_API_Que where itemid in (" & arrItemid & ") and mallid = '11st1010' "
	dbget.Execute sqlStr,AssignedRow
...
<script type="text/javascript">
	...
	function arrSubmit(ino){
        ...
        document.frmSvArr.action = '/outmall/11st/11stActProc.asp';
