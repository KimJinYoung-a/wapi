...
Class AESEncrypter
	...
	'키값. 서버간에 동일해야 한다. 보안을 위해 자주 변경한다.
	Private AES_KEY_STRING
	...
	Private Sub Class_Initialize()
		...
		'키값. 보안을 위해 자주 변경한다.
		AES_KEY_STRING = "TenbyTen AES key fdsf7gfdgfd9fhds"
...
'
'		AESEncrypt = EncryptData(msg, AES_KEY_STRING)
'
'	end function
'
'	function AESDecrypt(encryptedmsg)
'
'		AESDecrypt = DecryptData(encryptedmsg, AES_KEY_STRING)
'
'	end function

	function ConvertByteToString(msg)
		...
		For i = 0 To UBound(msg)
		    tmp = tmp & Right("0" & Hex(msg(i)), 2)
		Next

		ConvertByteToString = tmp

	end function

	function ConvertStringToByte(msg)

		dim tmp()
		dim i

		redim tmp(len(msg)/2 - 1)
		for i = 1 to (len(msg))
		    tmp(((i + 1) / 2) - 1) = cbyte("&H" & mid(msg, i, 2))
		    i = i + 1
		next

		ConvertStringToByte = tmp
	...
	function AESEncrypt(msg)
		...
		for lcount = 1 to llength
		    bytin(lcount - 1) = cbyte(ascb(mid(msg, lcount, 1)))
		next

		llength = len(key)
		redim bytpassword(llength - 1)
		for lcount = 1 to llength
		    bytpassword(lcount - 1)=cbyte(ascb(mid(key, lcount, 1)))
		next

		AESEncrypt = ConvertByteToString(EncryptData(bytin, bytpassword))
	...
	function AESDecrypt(encryptedmsg)
		...
		encryptedmsg = ConvertStringToByte(encryptedmsg)
		key = AES_KEY_STRING
		...
		bytclear = DecryptData(encryptedmsg, bytpassword)
	    ...
	    ''AESDecrypt = Base64decodeUnicode(stemp)
	    AESDecrypt = stemp
...
''dim result
''
''set aes = new AESEncrypter
''
''result = aes.AESEncrypt(msg)
''
''set aes = nothing
''
'''// ===========================================================================
...
''set aes = new AESEncrypter
''
''result = aes.AESDecrypt(encryptedmsg)
''
''set aes = nothing
