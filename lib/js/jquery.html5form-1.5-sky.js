...
(function($){
	...
	// prepare for HTML5 FORM
	$.fn.prepareHTML5 = function(options){
    ...
    $.fn.validate = function(){
        ...
        $(this).each(function(){
			...
			$(required).each(function(key, value) {
				...
				if(($(this).val()==$(this).attr('placeholder')) || ($(this).val()=='')){
					...
					if(opts.emptyMessage){
						...
						if (opts.responseDiv != null) {
							$(opts.responseDiv).html('<p>'+opts.emptyMessage+'</p>');
						} else {
							var msg = opts.emptyMessage.replace("#", $(this).attr('title'));
							alert(msg);
						}
					}
			...
			if(!emptyInput && !emailError){
				...
				if (opts.moreCheckFunction != null) {
					var checkFunction = eval(opts.moreCheckFunction);
					if (eval(checkFunction) != true) {
						return false;
					}
				}
