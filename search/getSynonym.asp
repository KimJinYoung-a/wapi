...
dim reBuf
dim q : q = request("word")
dim idomainno : idomainno = request("dno")
if (idomainno="") then idomainno="0"
...
ret_synonym = oDoc.GetSynonymListWithDomain(idomainno)
ret_synonym = RTrim(ret_synonym)
SET oDoc=Nothing


Dim retArr : retArr = split(ret_synonym,vbCrlf)
Dim retSubArr
...
if UBound(retArr)>-1 then
    ...
    For i=LBound(retArr) To UBound(retArr)
        retSubArr = split(retArr(i),",")
        For j=LBound(retSubArr) To UBound(retSubArr)
            Set oDataItem = jsObject()
            oDataItem("position") = k+1
            oDataItem("termposition") = i+1
            oDataItem("word") = retSubArr(j)
