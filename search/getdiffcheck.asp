...
function CheckVaildIP(ref)
    ...
    dim VaildIP : VaildIP = Array("13.125.145.40","13.125.12.181","52.79.73.145","61.252.133.88","192.168.1.70","192.168.1.72","110.93.128.107","61.252.133.2","61.252.133.69","61.252.133.70","61.252.133.80","61.252.143.71","61.252.133.12","110.93.128.114","110.93.128.113","61.252.133.72")
    dim i
...
dim SellScope : SellScope = "" ' 판매여부 Y:판매상품만  ELSE 일시품절 포함.
dim SortMet : SortMet="be" ''"be"
...
dim retTTLcnt, chkitemid, duppcnt
dim sellcash1, sellcash2, avgsellcash1, avgsellcash2
retTTLcnt = oTotalCnt.FTotalcount
...
IF oDoc.FResultCount >0 then
    For i=0 To oDoc.FResultCount -1
        ...
        sellcash1 = sellcash1 + oDoc.FITemList(i).FSellcash
    Next
end if
...
IF oDoc.FResultCount >0 then
    ...
    For i=0 To oDoc.FResultCount -1
        chkitemid = oDoc.FITemList(i).FItemid
        sellcash2 = sellcash2 + oDoc.FITemList(i).FSellcash
        reBuf2 = reBuf2 &chkitemid&","
        if InStr(reBuf,chkitemid&",")>0 then
            duppcnt = duppcnt+1
        end if
    Next
end if
...
if oDoc.FResultCount>0 then
    avgsellcash1 = CLNG(sellcash1/ oDoc.FResultCount)
    avgsellcash2 = CLNG(sellcash2/ oDoc.FResultCount)
else
    avgsellcash1 = 0
    avgsellcash2 = 0
end if
...
response.write retTTLcnt&vbTab&duppcnt&vbTab&avgsellcash1&vbTab&avgsellcash2
